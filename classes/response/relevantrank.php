...
use mod_smartquest\db\bulk_sql_config;
...
class relevantrank extends base {
    static public function response_table() {
        return 'smartquest_response_relrank';
    }
    ...
    public function insert_response($rid, $val) {
        global $DB;
        if ($this->question->type_id == QUESRELEVANTRATE) {
            $resid = false;
            foreach ($this->question->choices as $cid => $choice) {
                ...
                if ($other == get_string('notapplicable', 'smartquest')) {
                    $rank = -1;
                    $reason = optional_param('nrq'.$this->question->id.'_'.$cid, null, PARAM_RAW);
                } else {
                    $rank = intval($other);
                    if ($rank == -1) {
                        $reason = optional_param('nrq'.$this->question->id.'_'.$cid, null, PARAM_RAW);
    ...
    public function get_results($rids=false, $anonymous=false) {
        ...
        if ($this->question->type_id == QUESRELEVANTRATE) {
            ...
            if ($rows = $DB->get_records_select('smartquest_quest_choice', $select)) {
                foreach ($rows as $row) {
            ...
            if (!$isrestricted) {
                if (!empty ($rankvalue)) {
                    ...
                    FROM {smartquest_quest_choice} c, {".self::response_table()."} r
                ...
                $sql = "SELECT c.id, c.content, a.average, a.num
                        ...
                        INNER JOIN
                             (SELECT c2.id, AVG(a2.rank+1) AS average, COUNT(a2.response_id) AS num
                              FROM {smartquest_quest_choice} c2, {".self::response_table()."} a2
    ...
    public function display_results($rids=false, $sort='', $anonymous=false) {
        ...
        if ($rows = $this->get_results($rids, $sort, $anonymous)) {
                ...
                if (array_key_exists($ccontent, $rows)) {
                    ...
                    if (isset($rows[$ccontent]->averagevalue)) {
                        ...
                        if ($stravgvalue == '' && !$osgood) {
                            $stravgvalue = ' ('.get_string('andaveragevalues', 'smartquest').')';
                        }
                    } else {
                        $avgvalue = null;
                    }
            ...
            $output .= \mod_smartquest\response\display_support::mkresavg($this->counts, count($rids), $this->question->choices,
                $this->question->precise, $prtotal, $this->question->length, $sort, $stravgvalue);

            $output .= \mod_smartquest\response\display_support::mkrescount($this->counts, $rids, $rows, $this->question,
                $this->question->precise, $this->question->length, $sort);
        } else {
            $output .= '<p class="generaltable">&nbsp;'.get_string('noresponsedata', 'smartquest').'</p>';
        }
        return $output;
    }
    ...
    static public function response_select($rid, $col = null, $csvexport = false, $choicecodes = 0, $choicetext = 1) {
        ...
        $sql = 'SELECT a.id as aid, q.id AS qid, q.precise AS precise, c.id AS cid '.$col.', c.content as ccontent,
                                a.rank as arank, a.reason as reason '.
            'FROM {'.self::response_table().'} a, {smartquest_question} q, {smartquest_quest_choice} c '.
                ...
                if ($key != 'content') { // No need to keep question text - ony keep choice text and rank.
                    if ($key == 'ccontent') {
                        if ($osgood) {
                            ...
                            $contents = smartquest_choice_values($val);
                            if ($contents->modname) {
