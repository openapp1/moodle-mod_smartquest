...
namespace mod_smartquest\response;
...
class display_support {
    ...
    public static function mkrespercent($weights, $total, $precision, $showtotals, $sort) {
        ...
        $i = 0;
        $alt = '';
        $imageurl = $CFG->wwwroot.'/mod/smartquest/images/';
        $strtotal = get_string('total', 'smartquest');
        ...
        $table->head = array_merge($table->head, array(get_string('response', 'smartquest'),
                       get_string('average', 'smartquest'), get_string('total', 'smartquest')));
	    ...
	    //>
            ...
            $tabledata = array();
            $tabledata = array_merge($tabledata, array('', get_string('noresponsedata', 'smartquest')));
            $table->data[] = $tabledata;
        }
    ...
    public static function mkreslisttext($rows) {
        global $CFG, $SESSION, $smartquest, $DB;
        $strresponse = get_string('response', 'smartquest');
        $viewsingleresponse = $smartquest->capabilities->viewsingleresponse;
        $nonanonymous = $smartquest->respondenttype != 'anonymous';
        $table = new html_table();
        if ($viewsingleresponse && $nonanonymous) {
            $strrespondent = get_string('respondent', 'smartquest');
            $table->align = array('left', 'left');
            $currentgroupid = '';
            if (isset($SESSION->smartquest->currentgroupid)) {
                $currentgroupid = $SESSION->smartquest->currentgroupid;
            }
            $url = $CFG->wwwroot.'/mod/smartquest/report.php?action=vresp&amp;sid='.$smartquest->survey->id.
    ...
    public static function mkreslistdate($counts, $total, $precision, $showtotals) {
        ...
        $strresponse = get_string('response', 'smartquest');
        $strnum = get_string('num', 'smartquest');
        $table = new html_table();
    ...
    public static function mkreslistnumeric($counts, $total, $precision) {
        ...
        $strresponse = get_string('response', 'smartquest');
        $strnum = get_string('num', 'smartquest');
        $strnoresponsedata = get_string('noresponsedata', 'smartquest');
        $straverage = get_string('average', 'smartquest');
        $table = new html_table();
        $table->align = array('left', 'right');
        $table->head = array($strnum, $strresponse);
        $table->size = array('*', '*');
    ...
    public static function mkresavg($counts, $total, $choices, $precision, $showtotals, $length, $sort, $stravgvalue='') {
        global $CFG;
        $stravgrank = get_string('averagerank', 'smartquest');
        $osgood = false;
        if ($precision == 3) { // Osgood's semantic differential.
            $osgood = true;
            $stravgrank = get_string('averageposition', 'smartquest');
        }
        ...
        if ($isna) {
            $isnahead = get_string('notapplicable', 'smartquest');
        }
        $table = new html_table();
    ...
    public static function mkrescount($counts, $rids, $rows, $question, $precision, $length, $sort) {
        ...
        $sql = 'SELECT r.id, c.content, r.rank, c.id AS choiceid ' .
                'FROM {smartquest_quest_choice} c , ' .
                     '{smartquest_response_rank} r ' .
                'WHERE c.question_id = ?' .
        ...
        $strtotal = '<strong>'.get_string('total', 'smartquest').'</strong>';
        $isna = $precision == 1;
        $isnahead = '';
        $osgood = false;
        if ($precision == 3) { // Osgood's semantic differential.
        ...
        if ($isna) {
            $isnahead = get_string('notapplicable', 'smartquest').'<br />(#)';
        }
        if ($precision == 1) {
        ...
        $headings = array('<span class="smalltext">'.get_string('responses', 'smartquest').'</span>');
        if ($osgood) {
            $align = array('right');
        } else {
            $align = array('left');
        ...
        if ($isrestricted) {
            array_push($headings, get_string('notapplicable', 'smartquest'));
            array_push($align, 'center');
        }
        array_push($align, 'center');
        if ($na) {
