...
class single extends base {
    ...
    public function get_results($rids=false, $anonymous=false) {
        ...
        $sql = 'SELECT rt.id, qc.id as cid, qc.content ' .
               'FROM {smartquest_quest_choice} qc, ' .
        ...
        $sql = 'SELECT rt.id, rt.response, qc.content ' .
               'FROM {smartquest_response_other} rt, ' .
                    '{smartquest_quest_choice} qc ' .
    ...
    static public function response_select($rid, $col = null, $csvexport = false, $choicecodes = 0, $choicetext = 1) {
        ...
        foreach ($records as $qid => $row) {
            ...
            if ($csvexport) {
                static $i = 1;
                $qrecords = $DB->get_records('smartquest_quest_choice', ['question_id' => $qid]);
                foreach ($qrecords as $value) {
                    if ($value->id == $cid) {
                        $contents = smartquest_choice_values($value->content);
                        if ($contents->modname) {
                            $row->ccontent = $contents->modname;
                        } else {
                            $content = $contents->text;
                            if (preg_match('/^!other/', $content)) {
                                $row->ccontent = get_string('other', 'smartquest');
    ...
    public function get_bulk_sql($surveyid, $responseid = false, $userid = false, $groupid = false) {
      ...
      LEFT JOIN {smartquest_response_other} qro ON qro.response_id = qr.id AND qro.choice_id = qrs.choice_id
