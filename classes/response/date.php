...
class date extends base {
    static public function response_table() {
        return 'smartquest_response_date';
    }

    public function insert_response($rid, $val) {
        global $DB;
        $checkdateresult = smartquest_check_date($val);
        $thisdate = $val;
        if (substr($checkdateresult, 0, 5) == 'wrong') {
            return false;
        }
        // Now use ISO date formatting.
        $checkdateresult = smartquest_check_date($thisdate, true);
        $record = new \stdClass();
    ...
    public function display_results($rids=false, $sort='', $anonymous=false) {
        ...
        if ($rows = $this->get_results($rids, $anonymous)) {
            ...
            $output .= \mod_smartquest\response\display_support::mkreslistdate($this->counts, count($rids),
    ...
    static public function response_select($rid, $col = null, $csvexport = false, $choicecodes = 0, $choicetext = 1) {
        ...
        $sql = 'SELECT q.id '.$col.', a.response as aresponse '.
            'FROM {'.self::response_table().'} a, {smartquest_question} q '.
        ...
        foreach ($records as $qid => $row) {
            ...
            foreach ($row as $key => $val) {
                if (!is_numeric($key)) {
                    $newrow[] = $val;
                    // Convert date from yyyy-mm-dd database format to actual smartquest dateformat.
