...
abstract class base {
            ...
            foreach ($rows as $idx => $row) {
                if (strpos($idx, 'other') === 0) {
                    ...
                    $content = preg_replace(array('/^!other=/', '/^!other/'),
                            array('', get_string('other', 'smartquest')), $ccontent);
                    $content .= ' ' . clean_text($answer);
                    $textidx = $content;
                    $this->counts[$textidx] = !empty($this->counts[$textidx]) ? ($this->counts[$textidx] + 1) : 1;
                } else {
                    $contents = smartquest_choice_values($row->content);
                    $this->choice = $contents->text.$contents->image;
                    $textidx = $this->choice;
                    $this->counts[$textidx] = !empty($this->counts[$textidx]) ? ($this->counts[$textidx] + 1) : 1;
                }
            }
            $output .= \mod_smartquest\response\display_support::mkrespercent($this->counts, count($rids),
    ...
    /**
     * Return all the fields to be used for users in bulk smartquest sql.
     *
     * @author: Guy Thomas
     * @return string
     */
    ...
    protected function bulk_sql_config() {
        return new bulk_sql_config('smartquest_response_other', 'qro', true, true, false);
    }
