...
class multiple extends single {
    ...
    static public function response_table() {
        return 'smartquest_resp_multiple';
    }
        ...
        foreach ($this->question->choices as $cid => $choice) {
            if (strpos($choice->content, '!other') === 0) {
                ...
                if (preg_match("/[^ \t\n]/", $other)) {
                    ...
                    $resid = $DB->insert_record('smartquest_response_other', $record);
                }
            }
        }
    ...
    static public function response_select($rid, $col = null, $csvexport = false, $choicecodes = 0, $choicetext = 1) {
        ...
        if ($csvexport) {
            ...
            if (!empty($records)) {
                ...
                $sql = 'SELECT * FROM {smartquest_quest_choice} WHERE question_id ' . $qsql . ' ORDER BY id';
                ...
                foreach ($records2 as $qid => $row2) {
                    ...
                    $otherend = false;
                    if ($c2 == '!other') {
                        $c2 = '!other='.get_string('other', 'smartquest');
                    }
                    if (preg_match('/^!other/', $c2)) {
                        $otherend = true;
                    } else {
                        $contents = smartquest_choice_values($c2);
                        if ($contents->modname) {
                            $c2 = $contents->modname;
                    ...
                    $sql = 'SELECT a.name as name, a.type_id as q_type, a.position as pos ' .
                        'FROM {smartquest_question} a WHERE id = ?';
                    ...
                    if ($otherend) {
                        ...
                        $newrow2[] = $type1;
                        $newrow2[] = $name1;
                        $newrow2[] = '['.get_string('other', 'smartquest').']';
                        $newrow2[] = $selected;
                        $tmp2 = $qid2.'_other';
                        $values["$tmp2"] = $newrow2;
                    }
        ...
        $sql = 'SELECT c.id as cid, c.content as content, a.response as aresponse, q.id as qid, q.position as position,
            ...
            'FROM {smartquest_response_other} a, {smartquest_question} q, {smartquest_quest_choice} c '.
            'WHERE a.response_id= ? AND a.question_id=q.id AND a.choice_id=c.id '.
    ...
    public function get_bulk_sql($surveyid, $responseid = false, $userid = false, $groupid = false) {
      ...
      LEFT JOIN {smartquest_response_other} qro ON qro.response_id = qr.id AND qro.choice_id = qrm.choice_id
    ...
    protected function bulk_sql() {
        ...
        return "
            SELECT " . $DB->sql_concat_join("'_'", ['qr.id', "'".$this->question->helpname()."'", $alias.'.id']) . " AS id,
                qr.aboutuserid, qr.submitted, qr.complete, qr.grade, qr.userid, $userfields, qr.id AS rid, $alias.question_id,
                   $extraselect
