...
class edit_question_form extends \moodleform {
    ...
    public function validation($data, $files) {
        ...
        if ($data['type_id'] == QUESRATE) {
            if ($data['length'] < 2) {
                $errors["length"] = get_string('notenoughscaleitems', 'smartquest');
            }
            ...
            if ($data['precise'] == 2 ) {
                ...
                if ($nbvalues < 2) {
                    $errors["allchoices"] = get_string('noduplicateschoiceserror', 'smartquest');
                }
            }
        }
        if ($data['type_id'] == QUESRELEVANTRATE) {
            if ($data['length'] < 2) {
                $errors["length"] = get_string('notenoughscaleitems', 'smartquest');
            }
            // If this is a rate question with no duplicates option.
