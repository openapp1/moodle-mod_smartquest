...
class essay extends base {
    ...
    protected function responseclass() {
        return '\\mod_smartquest\\response\\text';
    }
    ...
    protected function question_survey_display($data, $descendantsdata, $blanksmartquest=false) {
        ...
        if ($canusehtmleditor) {
            ...
            $editor->use_editor($name, smartquest_get_editor_options($this->context));
    ...
    protected function form_length(\MoodleQuickForm $mform, $helptext = '') {
        $responseformats = array(
                        "0" => get_string('formateditor', 'smartquest'),
                        "1" => get_string('formatplain', 'smartquest'));
        $mform->addElement('select', 'precise', get_string('responseformat', 'smartquest'), $responseformats);
    ...
    protected function form_precise(\MoodleQuickForm $mform, $helptext = '') {
        ...
        for ($lines = 5; $lines <= 40; $lines += 5) {
            $choices[$lines] = get_string('nlines', 'smartquest', $lines);
        }
        $mform->addElement('select', 'length', get_string('responsefieldlines', 'smartquest'), $choices);
