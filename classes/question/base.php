...
namespace mod_smartquest\question;
use mod_smartquest\edit_question_form;
use \smartquest;
...
/**
 * This file contains the parent class for smartquest question types.
 *
...
abstract class base {
    ...
    /** @var int $position Position in the smartquest */
    public $position    = 0;
    ...
    /** @var int $sourceid The parent question. */
    public $sourceid     = 0;
    ...
    public function __construct($id = 0, $question = null, $context = null, $params = []) {
        ...
        if ($qtypes === null) {
            $qtypes = $DB->get_records('smartquest_question_type', [], 'typeid',
                                       'typeid, type, has_choices, response_table');
        }

        if ($id) {
            $question = $DB->get_record('smartquest_question', ['id' => $id]);
        }
    ...
    static public function question_builder($qtype, $qdata = null, $context = null) {
        $qclassname = '\\mod_smartquest\\question\\'.self::qtypename($qtype);
        $qid = 0;
    ...
    private function get_choices() {
        ...
        if ($choices = $DB->get_records('smartquest_quest_choice', ['question_id' => $this->id], 'id ASC')) {
    ...
    private function get_dependencies() {
        ...
        $this->dependencies = [];
        $dependencies = $DB->get_records('smartquest_dependency',
    ...
    protected function get_dependency_options() {
        ...
        if ($this->allows_dependents() && $this->has_choices()) {
            foreach ($this->choices as $key => $choice) {
                $contents = smartquest_choice_values($choice->content);
    ...
    public function insert_response($rid, $val) {
        if (isset ($this->response) && is_object($this->response) &&
            is_subclass_of($this->response, '\\mod_smartquest\\response\\base')) {
     ...
     *
     * @return object The response object based off of smartquest_response_base.
     *
     */
    ...
    public function update($questionrecord = null, $updatechoices = true) {
        ...
        $DB->update_record('smartquest_question', $questionrecord);
    ...
    public function add($questionrecord, array $choicerecords = null, $calcposition = true) {
        ...
        $this->qid = $DB->insert_record('smartquest_question', $questionrecord);
    ...
    public function update_choice($choicerecord) {
        global $DB;
        return $DB->update_record('smartquest_quest_choice', $choicerecord);
    }

    public function add_choice($choicerecord) {
        global $DB;
        $retvalue = true;
        if ($cid = $DB->insert_record('smartquest_quest_choice', $choicerecord)) {
    ...
    public function delete_choice($choice) {
        ...
        if ($DB->delete_records('smartquest_quest_choice', ['id' => $cid])) {
    ...
    public function update_dependency($dependencyrecord) {
        global $DB;
        return $DB->update_record('smartquest_dependency', $dependencyrecord);
    }
    ...
    public function add_dependency($dependencyrecord) {
        ...
        $retvalue = true;
        if ($did = $DB->insert_record('smartquest_dependency', $dependencyrecord)) {
    ...
    public function delete_dependency($dependency) {
        ...
        if ($DB->delete_records('smartquest_dependency', ['id' => $did])) {
    ...
    public function set_required($required) {
        ...
        $this->required = $rval;
        return $DB->set_field('smartquest_question', 'required', $rval, ['id' => $qid]);
    }
     ...
     * @param boolean $blanksmartquest
     *
     */
    abstract protected function question_survey_display($formdata, $descendantsdata, $blanksmartquest);
     ...
     * @param boolean $blanksmartquest
     */
    public function question_output($formdata, $dependants=[], $qnum='', $blanksmartquest) {
        $pagetags = $this->questionstart_survey_display($qnum, $formdata);
        $pagetags->qformelement = $this->question_survey_display($formdata, $dependants, $blanksmartquest);
        return $pagetags;
    }
    ...
    public function questionstart_survey_display($qnum, $formdata='') {
        global $OUTPUT, $SESSION, $smartquest, $PAGE;
        ...
        // If we are on report page and this smartquest has dependquestions and this question was skipped.
        if (($pagetype == 'mod-smartquest-myreport' || $pagetype == 'mod-smartquest-report') &&
        ...
        // In report mode, If smartquest is set to no numbering,
        // also hide answers to questions that have not been answered.
        $displayclass = 'qn-container';
        if ($pagetype == 'mod-smartquest-preview' || ($nonumbering &&
            ...
            $smartquest->load_parents($this);
            // Want this to come from the renderer, meaning we need $smartquest.
            $pagetags->dependencylist = $smartquest->renderer->get_dependency_html($this->id, $this->dependencies);
        }
        ...
        if ($this->type_id != QUESSECTIONTEXT) {
            ...
            if ($this->required()) {
                ...
                $required .= get_string('required', 'smartquest');
                $required .= html_writer::end_tag('div');
                //$required .= html_writer::empty_tag('img', ['class' => 'req', 'title' => get_string('required', 'smartquest'),
                  //  'alt' => get_string('required', 'smartquest'), 'src' => $OUTPUT->image_url('req')]);
            }
        ...
        $content = format_text(file_rewrite_pluginfile_urls($this->content, 'pluginfile.php',
            $this->context->id, 'mod_smartquest', 'question', $this->id), FORMAT_HTML, $options);
    ...
    private function response_check_required ($data) {
        ...
        if ($this->type_id == QUESRELEVANTRATE) { // Rate is a special case.
            foreach ($this->choices as $cid => $choice) {
                $str = 'q'."{$this->id}_$cid";
                if (isset($data->$str)) {
                    return ('&nbsp;');
     ...
     * @param smartquest $smartquest The smartquest being edited.
     * @return bool
     */
    public function edit_form(edit_question_form $form, smartquest $smartquest) {
        $mform =& $form->_form;
        ...
        if ($smartquest->navigate > 0) {
            $this->form_dependencies($form, $smartquest->questions);
        }
        ...
        if (isset($this->qid)) {
            $buttonarray[] = &$mform->createElement('submit', 'makecopy', get_string('saveasnew', 'smartquest'));
        }
    ...
    protected function form_header(\MoodleQuickForm $mform, $helpname = '') {
        ...
        if (isset($this->qid) && !empty($this->qid)) {
            $header = get_string('editquestion', 'smartquest', smartquest_get_type($this->type_id));
        } else {
            $header = get_string('addnewquestion', 'smartquest', smartquest_get_type($this->type_id));
        }
        if (empty($helpname)) {
        ...
        $mform->addHelpButton('questionhdredit', $helpname, 'smartquest');
    }

    protected function form_name(\MoodleQuickForm $mform) {
        $mform->addElement('text', 'name', get_string('optionalname', 'smartquest'),
                        ['size' => '30', 'maxlength' => '30']);
        $mform->setType('name', PARAM_TEXT);
        $mform->addHelpButton('name', 'optionalname', 'smartquest');
        return $mform;
    }
    ...
    protected function form_required(\MoodleQuickForm $mform) {
        ...
        $mform->addGroup($reqgroup, 'reqgroup', get_string('required', 'smartquest'), ' ', false);
        $mform->addHelpButton('reqgroup', 'required', 'smartquest');
        return $mform;
    }
    ...
    protected function form_dependencies($form, $questions) {
        ...
        if (count($dependencies) > 1) {
            $mform->addElement('header', 'dependencies_hdr', get_string('dependencies', 'smartquest'));
             ...
             * bottom up, starting at the lowest child question is a pain for large smartquests.
            ...
            $mform->addElement('static', 'mandatory', '',
                '<div class="dimmed_text">' . get_string('mandatory', 'smartquest') . '</div>');
            $selectand = $mform->createElement('select', 'dependlogic_and', get_string('condition', 'smartquest'),
                [get_string('answernotgiven', 'smartquest'), get_string('answergiven', 'smartquest')]);
            $selectand->setSelected('1');
            $groupitemsand = [];
            $groupitemsand[] =& $mform->createElement('selectgroups', 'dependquestions_and',
                get_string('parent', 'smartquest'), $dependencies);
            $groupitemsand[] =& $selectand;
            $groupand = $mform->createElement('group', 'selectdependencies_and', get_string('dependquestion', 'smartquest'),
                $groupitemsand, ' ', false);
            ...
            $mform->addElement('static', 'optional', '',
                '<div class="dimmed_text">' . get_string('optional', 'smartquest') . '</div>');
            $selector = $mform->createElement('select', 'dependlogic_or', get_string('condition', 'smartquest'),
                [get_string('answernotgiven', 'smartquest'), get_string('answergiven', 'smartquest')]);
            ...
            $groupor = $mform->createElement('group', 'selectdependencies_or', get_string('dependquestion', 'smartquest'),
                $groupitemsor, ' ', false);
    ...
    protected function form_question_text(\MoodleQuickForm $mform, $context) {
        ...
        $mform->addElement('editor', 'content', get_string('text', 'smartquest'), null, $editoroptions);
    ...
    protected function form_choices(\MoodleQuickForm $mform, array $choices, $helpname = '') {
        ...
        $mform->addElement('textarea', 'allchoices', get_string('possibleanswers', 'smartquest'), $options);
        $mform->setType('allchoices', PARAM_RAW);
        $mform->addRule('allchoices', null, 'required', null, 'client');
        $mform->addHelpButton('allchoices', $helpname, 'smartquest');
    ...
    static public function form_length_text(\MoodleQuickForm $mform, $helpname = '', $value = 0) {
        $mform->addElement('text', 'length', get_string($helpname, 'smartquest'), ['size' => '1'], $value);
        $mform->setType('length', PARAM_INT);
        if (!empty($helpname)) {
            $mform->addHelpButton('length', $helpname, 'smartquest');
        }
        return $mform;
    }
    ...
    static public function form_precise_text(\MoodleQuickForm $mform, $helpname = '', $value = 0) {
        $mform->addElement('text', 'precise', get_string($helpname, 'smartquest'), ['size' => '1']);
        $mform->setType('precise', PARAM_INT);
        if (!empty($helpname)) {
            $mform->addHelpButton('precise', $helpname, 'smartquest');
        }
        return $mform;
    }
     ...
     */
    public function form_update($formdata, $smartquest) {
        global $DB;
        ...
        if (!empty($formdata->qid)) {
            ...
            $formdata->content = file_save_draft_area_files($formdata->itemid, $smartquest->context->id, 'mod_smartquest',
            ...
            if ($smartquest->has_dependencies()) {
            ...
            $formdata->format  = $formdata->content['format'];
            $formdata->content = $formdata->content['text'];
            $content           = file_save_draft_area_files($formdata->itemid, $smartquest->context->id, 'mod_smartquest',
                'question', $this->qid, ['subdirs' => true], $formdata->content);
            $DB->set_field('smartquest_question', 'content', $content, ['id' => $this->qid]);
        }
    ...
    protected function form_preprocess_choicedata($formdata) {
        if (empty($formdata->allchoices)) {
            error (get_string('enterpossibleanswers', 'smartquest'));
        }
        return false;
    }
