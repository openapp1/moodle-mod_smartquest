...
/**
 * This file contains the parent class for relevantrate question types.
 *
...
class relevantrate extends base {
    ...
    protected function responseclass() {
        return '\\mod_smartquest\\response\\relevantrank';
    }

    public function helpname() {
        return 'relevantratescale';
    }
    ...
    public function question_template() {
        return 'mod_smartquest/question_relevantrate';
    }
    ...
    public function response_template() {
        return 'mod_smartquest/response_relevantrate';
    }
    ...
    public function get_feedback_maxscore() {
        if ($this->valid_feedback()) {
            ...
            // The maximum score needs to be multiplied by the number of items to relevantrate.
     ...
     *
     */
    protected function question_survey_display($data, $descendantsdata, $blanksmartquest=false) {
        global $PAGE;

        foreach ($this->choices as $cid => $choice) {
            $str = 'q'."{$this->id}_$cid";
            if (isset($data->$str) && !is_array($data->$str)) {
                $strnr = 'nrq'."{$this->id}_$cid";
                $arr = [];
                $arr[1] = $data->$str;
                $arr[2] = $data->$strnr;
                $data->$str = $arr;
        ...
        // $PAGE->requires->js('/mod/smartquest/javascript/relevantrate.js');
        ...
        // Check if relevantrate question has one line only to display full width columns of choices.
        $nocontent = false;
        $nameddegrees = 0;
        $n = array();
        $v = array();
        ...
        // A one-line relevantrate question (no content) does not need to span more than 50%.
        ...
        if ($isna) {
            $na = get_string('notapplicable', 'smartquest');
        } else {
            $na = '';
        }
        if ($this->precise == 2) {
            $order = 'other_relevantrate_uncheck(name, value)';
        } else {
            $order = '';
        }
        ...
        } else { // If "No duplicate choices", can restrict nbchoices to number of relevantrate items specified.
            $nbchoices = $this->length;
        }

        // Display empty td for Not yet answered column.
        if ($nbchoices > 1 && $this->precise != 2 && !$blanksmartquest) {
        ...
        if ($na) {
            ...
            $collabel[$j] = $na;
	        $choicetags->qelements['headerrow']['cols'][] = ['colwidth' => $colwidth, 'coltext' => get_string('explainnotapplicable', 'smartquest')];
        }
        $num = 0;
        foreach ($this->choices as $cid => $choice) {
            $str = 'q'."{$this->id}_$cid";
            $num += (isset($data->$str[1]) && ($data->$str[1] != -999));
        }

        $notcomplete = false;
        if ( ($num != $nbchoices) && ($num != 0) ) {
            $this->add_notification(get_string('checkallradiobuttons', 'smartquest', $nbchoices));
            $notcomplete = true;
        }
        ...
        foreach ($this->choices as $cid => $choice) {
            ...
            if (isset($choice->content)) {
                ...
                $bg = 'c0 relevantraterow';
                if ($nbchoices > 1 && $this->precise != 2  && !$blanksmartquest) {
                    $checked = ' checked="checked"';
                    $completeclass = 'notanswered';
                    $title = '';
                    if ($notcomplete && isset($data->$str[1]) && ($data->$str[1] == -999)) {
                        $completeclass = 'notcompleted';
                        $title = get_string('pleasecomplete', 'smartquest');
                    }
                    ...
                    $cols[] = ['colstyle' => 'width:1%;', 'colclass' => $completeclass, 'coltitle' => $title,
                        'colinput' => $colinput, 'type' => 'radio'];
                }
                for ($j = 0; $j < $this->length + $isna; $j++) {
                    $col = [];
                    $checked = ((isset($data->$str[1]) && ($j == $data->$str[1] ||
                                 $j == $this->length && $data->$str[1] == -1)) ? ' checked="checked"' : '');
                    $checked = '';
                    if (isset($data->$str[1]) && ($j == $data->$str[1] || $j == $this->length && $data->$str[1] == -1)) {
                        $checked = ' checked="checked"';
                    }
                    $col['colstyle'] = 'text-align:center';
                    $col['colclass'] = $bg;
                    $i = $j + 1;
                    $col['colhiddentext'] = get_string('option', 'smartquest', $i);
                ...
                if (!empty($data->$str[2])) {
               	    $col = [];
               	    $col['colclass'] = 'notrelevant';
               	    $col['colinput']['name'] = 'nr'.$str;
                    $col['colinput']['value'] = $data->$str[2];
                    $col['colinput']['id'] = 'nr'.$str;
                    $col['colinput']['type'] = 'text';
                    $col['colinput']['label'] = 'Explain ';
                    $cols[] = $col;
                } else {
                    $col = [];
                    if (isset($nrc) && $nrc) {
                        $col['colclass'] = 'notcompleted';
                        $this->add_notification(get_string('checkrelallradiobuttons', 'smartquest'));
                    } else {
                        //$col['colstyle'] = 'display:none';
                        $col['inputstyle'] = 'visibility:hidden';
                        $col['colclass'] = 'notrelevant';                                                                    
                    }
                    $col['colinput']['name'] = 'nr'.$str;
                    $col['colinput']['id'] = 'nr'.$str;
    ...
    /**
     * Return the context tags for the relevantrate response template.
     * @param object $data
     * @return object The relevantrate question response context tags.
     *
     */
    protected function response_survey_display($data) {
        ...
        if ($this->precise == 1) {
            $cellobj = new \stdClass();
            $cellobj->bg = $bg;
            $cellobj->str = get_string('notapplicable', 'smartquest');
            $resptags->headers[] = $cellobj;
            $cellobj = new \stdClass();
            $cellobj->bg = $bg;
            $cellobj->str = get_string('explainnotapplicable', 'smartquest');
            $resptags->headers[] = $cellobj;
        ...
        foreach ($this->choices as $cid => $choice) {
            ...
            if (!array_key_exists($cid, $cidnamed)) {
                ...
                for ($j = 0; $j < $this->length; $j++) {
                    $cellobj = new \stdClass();
                    if (isset($data->$str[1]) && ($j == $data->$str[1])) {
                        $cellobj->checked = 1;
                    }
                    $cellobj->str = $str.$j.$uniquetag++;
                    $cellobj->bg = $bg;
                    // N/A column checked.
                    $checkedna = (isset($data->$str[1]) && ($data->$str[1] == -1));
                    if ($bg == 'c0') {
                        $bg = 'c1';
                    } else {
                        $bg = 'c0';
    ...
    public function response_valid($responsedata) {
        ...
        foreach ($this->choices as $cid => $choice) {
            ...
            if (preg_match("/^[0-9]{1,3}=/", $content)) {
                ...
                $nrstr = 'nr'.$str;
                $num += (isset($responsedata->$str) && ($responsedata->$str != -999));
                $num += (isset($responsedata->$str) && ($responsedata->$str == -1) && empty($responsedata->$nrstr));
            }
            $nbchoices -= $nameddegrees;
        }
    ...
    protected function form_precise(\MoodleQuickForm $mform, $helptext = '') {
        $precoptions = array(/*"0" => get_string('normal', 'smartquest'),*/
                             "1" => get_string('notapplicablecolumn', 'smartquest'),
                             /*"2" => get_string('noduplicates', 'smartquest'),
                             "3" => get_string('osgood', 'smartquest')*/);
        $mform->addElement('select', 'precise', get_string('kindofrelevantratescale', 'smartquest'), $precoptions);
        $mform->addHelpButton('precise', 'kindofrelevantratescale', 'smartquest');
