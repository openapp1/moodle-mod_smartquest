...
class check extends base {
    ...
    protected function responseclass() {
        return '\\mod_smartquest\\response\\multiple';
    }
    ...
    public function question_template() {
        return 'mod_smartquest/question_check';
    }
    ...
    public function response_template() {
        return 'mod_smartquest/response_check';
    }
        ...
        if (!empty($data) ) {
            ...
            if ( $data->{'q'.$this->id} ) {
                ...
                if ($nbboxes < $min || $nbboxes > $max) {
                    $msg = get_string('boxesnbreq', 'smartquest');
                    if ($min == $max) {
                        $msg .= '&nbsp;'.get_string('boxesnbexact', 'smartquest', $min);
                    } else {
                        if ($min && ($nbboxes < $min)) {
                            $msg .= get_string('boxesnbmin', 'smartquest', $min);
                            if ($nbboxes > $max) {
                                $msg .= ' & ' .get_string('boxesnbmax', 'smartquest', $max);
                            }
                        } else {
                            if ($nbboxes > $max ) {
                                $msg .= get_string('boxesnbmax', 'smartquest', $max);
        ...
        foreach ($this->choices as $id => $choice) {
            ...
            if ($other !== 0) { // This is a normal check box.
                ...
                $othertext = preg_replace(
                        array("/^!other=/", "/^!other/"),
                        array('', get_string('other', 'smartquest')),
                        $choice->content);
                $cid = 'q'.$this->id.'_'.$id;
