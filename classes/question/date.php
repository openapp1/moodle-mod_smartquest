...
class date extends base {
    ...
    protected function responseclass() {
        return '\\mod_smartquest\\response\\date';
    }
    ...
    public function question_template() {
        return 'mod_smartquest/question_date';
    }
    ...
    public function response_template() {
        return 'mod_smartquest/response_date';
    }
    ...
    protected function question_survey_display($data, $descendantsdata, $blanksmartquest=false) {
        ...
        if (!empty($data->{'q'.$this->id})) {
            ...
            $setdate = smartquest_check_date ($dateentered, false);
            if ($setdate == 'wrongdateformat') {
                $msg = get_string('wrongdateformat', 'smartquest', $dateentered);
                $this->add_notification($msg);
            } else if ($setdate == 'wrongdaterange') {
                $msg = get_string('wrongdaterange', 'smartquest');
                $this->add_notification($msg);
            } else {
    ...
    public function response_valid($responsedata) {
        if (isset($responsedata->{'q'.$this->id})) {
            ...
            if ($responsedata->{'q'.$this->id} != '') {
                $checkdateresult = smartquest_check_date($responsedata->{'q'.$this->id});
            }
