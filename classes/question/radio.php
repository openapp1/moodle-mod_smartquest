...
class radio extends base {
    ...
    protected function responseclass() {
        return '\\mod_smartquest\\response\\single';
    }
    ...
    public function question_template() {
        return 'mod_smartquest/question_radio';
    }
    ...
    public function response_template() {
        return 'mod_smartquest/response_radio';
    }
     ...
     *
     */
    protected function question_survey_display($data, $dependants=[], $blanksmartquest=false) {
        // Radio buttons
        ...
        foreach ($this->choices as $id => $choice) {
            ...
            if ($other !== 0) { // This is a normal radio button.
                ...
                $othertext = preg_replace(["/^!other=/", "/^!other/"], ['', get_string('other', 'smartquest')],
                    $choice->content);
                $cid = 'q'.$this->id.'_'.$id;
                $otherempty = false;
        ...
        if (!$this->required()) {
            ...
            if (!$ischecked && !$blanksmartquest) {
                $radio->checked = true;
            }
            $content = get_string('noanswer', 'smartquest');
        ...
        if ($otherempty) {
            $this->add_notification(get_string('otherempty', 'smartquest'));
        }
        return $choicetags;
    }
    ...
    protected function form_length(\MoodleQuickForm $mform, $helptext = '') {
        $lengroup = [];
        $lengroup[] =& $mform->createElement('radio', 'length', '', get_string('vertical', 'smartquest'), '0');
        $lengroup[] =& $mform->createElement('radio', 'length', '', get_string('horizontal', 'smartquest'), '1');
        $mform->addGroup($lengroup, 'lengroup', get_string('alignment', 'smartquest'), ' ', false);
        $mform->addHelpButton('lengroup', 'alignment', 'smartquest');
