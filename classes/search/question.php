...
/**
 * Contains class mod_smartquest\search\question
 *
 ...
 */
namespace mod_smartquest\search;
defined('MOODLE_INTERNAL') || die();
/**
 * Search area for mod_smartquest questions. Separated from the activity search so that admins can choose whether or not they
...
class question extends \core_search\base_mod {
    ...
    public function get_recordset_by_timestamp($modifiedfrom = 0) {
        ...
        // Join the survey record to ensure only smartquests with questions are returned.
        $sql = 'SELECT q.* ' .
            'FROM {smartquest} q ' .
            'INNER JOIN {smartquest_survey} s ON q.sid = s.id ' .
     ...
     * questions associated with a smartquest.
     *
    ...
    public function get_document($record, $options = []) {
        ...
        try {
            $cm = $this->get_cm('smartquest', $record->id, $record->course);
        ...
        $recordset = $DB->get_recordset('smartquest_question', ['survey_id' => $record->sid, 'deleted' => 'n'],
            'id', 'id,content');
    ...
    public function check_access($id) {
        global $DB;
        try {
            $activity = $DB->get_record('smartquest', ['id' => $id], '*', MUST_EXIST);
            $cminfo = $this->get_cm('smartquest', $activity->id, $activity->course);
        ...
        // If the user has the ability to see questions beyond completing a smartquest, grant access.
        $context = \context_module::instance($cminfo->id);
        if (!(has_capability('mod/smartquest:readallresponses', $context) ||
              has_capability('mod/smartquest:readallresponseanytime', $context) ||
              has_capability('mod/smartquest:editquestions', $context))) {
            return \core_search\manager::ACCESS_DENIED;
        }
    ...
    public function get_context_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/smartquest/view.php', ['id' => $context->instanceid]);
    }
}