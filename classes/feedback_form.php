...
class feedback_form extends \moodleform {
    ...
    public function definition() {
        ...
        $this->_feedbacks = $DB->get_records('smartquest_feedback',
                array('section_id' => $sectionid),
                'minscore DESC');
        $this->context = $smartquest->context;
        $mform    =& $this->_form;

        if ($feedbacksections == 1) {
            $feedbackheading = get_string('feedbackglobalheading', 'smartquest');
            $feedbackmessages = get_string('feedbackglobalmessages', 'smartquest');
        } else {
            $feedbackheading = get_string('feedbacksectionheading', 'smartquest', $currentsection.'/'.$feedbacksections);
            $feedbackmessages = get_string('feedbackmessages', 'smartquest', $currentsection.'/'.$feedbacksections);
        }

        $mform->addElement('header', 'contenthdr', $feedbackheading);

        $questions = $smartquest->questions;
        $fbsection = $DB->get_record('smartquest_fb_sections',
                        array('survey_id' => $smartquest->survey->id, 'section' => $currentsection));
        $questionslist = '';
        ...
        $mform->addElement('static', 'questionsinsectionlist', get_string('questionsinsection', 'smartquest'), $questionslist);
        $mform->addElement('text', 'sectionlabel', get_string('feedbacksectionlabel', 'smartquest'),
                        array('size' => '50', 'maxlength' => '50'));
        $mform->setType('sectionlabel', PARAM_TEXT);
        $mform->addRule('sectionlabel', null, 'required', null, 'client');
        $mform->addHelpButton('sectionlabel', 'feedbacksectionlabel', 'smartquest');

        $editoroptions = array('maxfiles' => EDITOR_UNLIMITED_FILES, 'trusttext' => true);
        $mform->addElement('editor', 'sectionheading', get_string('feedbacksectionheadingtext', 'smartquest'),
                        null, $editoroptions);
        $mform->setType('info', PARAM_RAW);

        $mform->addHelpButton('sectionheading', 'feedbackheading', 'smartquest');

        // FEEDBACK FIELDS.

        $mform->addElement('header', 'feedbackhdr', $feedbackmessages);
        $mform->addHelpButton('feedbackhdr', 'feedback', 'smartquest');

        $mform->addElement('static', 'scoreboundarystatic1', get_string('feedbackscoreboundary', 'smartquest'), '100%');

        $repeatarray = array();
        $repeatedoptions = array();

        $repeatarray[] = $mform->createElement(
            'editor', 'feedbacktext', get_string('feedback', 'smartquest'), null,
            array('maxfiles' => EDITOR_UNLIMITED_FILES, 'noclean' => true, 'context' => $smartquest->context)
        );
        $repeatarray[] = $mform->createElement(
            'text', 'feedbackboundaries', get_string('feedbackscoreboundary', 'smartquest'), array('size' => 10)
        );
        ...
        $nextel = $this->repeat_elements($repeatarray, $numfeedbacks - 1,
                        ...
                        get_string('feedbackaddmorefeedbacks', 'smartquest'), true);

        // Put some extra elements in before the button.
        $mform->insertElementBefore(
            $mform->createElement('editor', "feedbacktext[$nextel]", get_string('feedback', 'smartquest'), null,
                array('maxfiles' => EDITOR_UNLIMITED_FILES, 'noclean' => true, 'context' => $smartquest->context)),
            'boundary_add_fields');
        $mform->insertElementBefore(
            $mform->createElement('static', 'scoreboundarystatic2', get_string('feedbackscoreboundary', 'smartquest'), '0%'),
            'boundary_add_fields');
        ...
        if ($currentsection < $feedbacksections) {
            ...
            $buttonarray[] = &$mform->createElement('submit', 'submitbutton',
                get_string('feedbacknextsection', 'smartquest', $sectionsnav));
        } else {
            $buttonarray[] = &$mform->createElement('submit', 'savesettings', get_string('savesettings', 'smartquest'));
        }
    ...
    public function validation($data, $files) {
        ...
        while (!empty($data['feedbackboundaries'][$i])) {
            ...
            if (is_numeric($boundary) && $boundary <= 0) {
                $errors["feedbackboundaries[$i]"] = get_string('feedbackerrorboundaryoutofrange', 'smartquest', $i + 1);
            }
            if (is_numeric($boundary) && $i > 0 &&
                    $boundary >= $data['feedbackboundaries'][$i - 1]) {
                $errors["feedbackboundaries[$i]"] = get_string('feedbackerrororder', 'smartquest', $i + 1);
            }
        ...
        if (!empty($data['feedbackboundaries'])) {
            for ($i = $numboundaries; $i < count($data['feedbackboundaries']); $i += 1) {
                if (!empty($data['feedbackboundaries'][$i] ) &&
                    ...
                    $errors["feedbackboundaries[$i]"] = get_string('feedbackerrorjunkinboundary', 'smartquest', $i + 1);
                }
            }
        }
        for ($i = $numboundaries + 1; $i < count($data['feedbacktext']); $i += 1) {
            if (!empty($data['feedbacktext'][$i]['text']) &&
                ...
                $errors["feedbacktext[$i]"] = get_string('feedbackerrorjunkinfeedback', 'smartquest', $i + 1);
            }
        }
        return $errors;
    }
