...
/**
 * Contains class mod_smartquest\output\renderer
 *
...
namespace mod_smartquest\output;
...
class renderer extends \plugin_renderer_base {
    ...
    public function render_viewpage($page) {
        ...
        return $this->render_from_template('mod_smartquest/viewpage', $data);
    }

    /**
     * Fill out the smartquest (complete) page.
     * @param \templateable $page
     * @return string | boolean
     */
    public function render_completepage($page) {
        $data = $page->export_for_template($this);
        return $this->render_from_template('mod_smartquest/completepage', $data);
    }
    ...
    public function render_reportpage($page) {
        $data = $page->export_for_template($this);
        return $this->render_from_template('mod_smartquest/reportpage', $data);
    }
    ...
    public function render_qsettingspage($page) {
        $data = $page->export_for_template($this);
        return $this->render_from_template('mod_smartquest/qsettingspage', $data);
    }
    ...
    public function render_questionspage($page) {
        $data = $page->export_for_template($this);
        return $this->render_from_template('mod_smartquest/questionspage', $data);
    }
    ...
    public function render_previewpage($page) {
        $data = $page->export_for_template($this);
        return $this->render_from_template('mod_smartquest/previewpage', $data);
    }
    ...
    public function render_nonrespondentspage($page) {
        $data = $page->export_for_template($this);
        return $this->render_from_template('mod_smartquest/nonrespondentspage', $data);
    }
    ...
    public function render_fbsectionspage($page) {
        $data = $page->export_for_template($this);
        return $this->render_from_template('mod_smartquest/fbsectionspage', $data);
    }
     ...
     * @param mod_smartquest\question\base $question The question object.
     ...
     * @param boolean $blanksmartquest Used for printing a blank one.
     * @return string The output for the page.
     */
    public function question_output($question, $formdata, $dependants=[], $qnum, $blanksmartquest) {

        $pagetags = $question->question_output($formdata, $dependants, $qnum, $blanksmartquest);
        ...
        return $this->render_from_template('mod_smartquest/question_container', $pagetags);
    }
            ...
            foreach ($data as $qnum => $responses) {
                ...
                foreach ($responses as $item => $response) {
                    if ($item !== 'question') {
                        ...
                        $resptags->respaboutuserid = $response['respaboutuserid'];
                        $pagetags->responses[] = $resptags;
                    }
                }
                $output .= $this->render_from_template('mod_smartquest/response_container', $pagetags);
            }
        }
        return $output;
    }
    ...
    public function navigationbar($navbar) {
        return $this->render_from_template('mod_smartquest/navbaralpha', $navbar);
    }
    ...
    public function usernavigationbar($navbar) {
        return $this->render_from_template('mod_smartquest/navbaruser', $navbar);
    }
    ...
    public function responselist($navbar) {
        return $this->render_from_template('mod_smartquest/responselist', $navbar);
    }
    ...
    public function dependency_warnings($children, $langstring, $strnum) {
        $msg = '<div class="warning">' . get_string($langstring, 'smartquest') . '</div><br />';
        foreach ($children as $child) {
            ...
            foreach ($child as $subchild) {
                ...
                switch ($subchild->dependlogic) {
                    case 0:
                        $logic = get_string('notset', 'smartquest');
                        break;
                    case 1:
                        $logic = get_string('set', 'smartquest');
                        break;
                    default:
                        $logic = '';
                }
                ...
                if (!in_array($subchild->qdependquestion, $loopindicator)) {
                    $msg .= '<div class = "qn-container">'.$strnum.' '.$subchild->position.$childname.
                        ...
                        get_string('dependquestion', 'smartquest').'</strong>'.
        ...
        foreach ($dependencies as $dependency) {
            ...
            if ($dependency->dependandor == "and") {
                $html .= '<div id="qdepend_' . $qid . '_' . $dependency->dependquestionid . '_' .
                    ...
                    get_string('dependquestion', 'smartquest') . '</strong> : '. get_string('position', 'smartquest') . ' ' .
