...
class smartquest {
    ...
    /**
     * @var \mod_smartquest\question\base[] $quesitons
     */
    public $questions = [];
     ...
     *
     */
    public function __construct($id = 0, $smartquest = null, &$course, &$cm, $addquestions = true) {
        global $DB, $smartquesttypenames;

        if ($id) {
            $smartquest = $DB->get_record('smartquest', array('id' => $id));
        }

        if (is_object($smartquest)) {
            $properties = get_object_vars($smartquest);
        ...
        $this->course = $course;
        $this->cm = $cm;
        // When we are creating a brand new smartquest, we will not yet have a context.
        ...
        // Load the capabilities for this user and smartquest, if not creating a new one.
        if (!empty($this->cm->id)) {
            $this->capabilities = smartquest_load_capabilities($this->cm->id);

            $view_cap_name = 'mod/smartquest:view' . $smartquesttypenames[$this->rtype] . 'smartquest';
            $roles = $DB->get_fieldset_select('role', 'id', '');
            $roles_with_cap = get_roles_with_capability($view_cap_name);

            foreach ($roles as $roleid) {
                $type = array_key_exists($roleid, $roles_with_cap) ? 1 : -1;
                // Assign_capability('mod/smartquest:view', $type, $roleid, $this->context, true);
            }
    ...
    public function add_survey($sid = 0, $survey = null) {
        ...
        if ($sid) {
            $this->survey = $DB->get_record('smartquest_survey', array('id' => $sid));
    ...
    public function add_questions($sid = false) {
        ...
        $params = [$sid, 'n'];
        if ($records = $DB->get_records_select('smartquest_question', $select, $params, 'position')) {
            $sec = 1;
            $isbreak = false;
            foreach ($records as $record) {

                $this->questions[$record->id] = \mod_smartquest\question\base::question_builder($record->type_id,
                    $record, $this->context);
    ...
    /**
     * Add the renderer to the smartquest object.
    ...
    /**
     * Add the templatable page to the smartquest object.
    ...
    public function view() {
        ...
        $PAGE->requires->js_init_call('M.mod_smartquest.init_attempt_form', null, false, smartquest_get_js_module());

        $smartquest = $this;

        if (!$this->capabilities->view) {
            $this->page->add_to_page('notifications',
                $this->renderer->notification(get_string('noteligible', 'smartquest', $this->name),
                \core\output\notification::NOTIFY_ERROR));
        } else if (!$this->is_active()) {
            $this->page->add_to_page('notifications',
                $this->renderer->notification(get_string('notavail', 'smartquest'), \core\output\notification::NOTIFY_ERROR));
        } else if (!$this->is_open()) {
            $this->page->add_to_page('notifications',
                $this->renderer->notification(get_string('notopen', 'smartquest', userdate($this->opendate)),
                \core\output\notification::NOTIFY_ERROR));
        } else if ($this->is_closed()) {
            $this->page->add_to_page('notifications',
                $this->renderer->notification(get_string('closed', 'smartquest', userdate($this->closedate)),
                \core\output\notification::NOTIFY_ERROR));
        } else if (!$this->user_is_eligible($USER->id)) {
            $this->page->add_to_page('notifications',
                $this->renderer->notification(get_string('noteligible', 'smartquest'), \core\output\notification::NOTIFY_ERROR));
        } else if ($this->survey->realm == 'template') {
            $this->page->add_to_page('notifications',
                $this->renderer->notification(get_string('templatenotviewable', 'smartquest'),
                \core\output\notification::NOTIFY_ERROR));
        } else if (!$this->is_complete_definition()) {
            $this->page->add_to_page('notifications',
                $this->renderer->notification(get_string('notcompletedefinition', 'smartquest'),
                \core\output\notification::NOTIFY_ERROR));
            ...
            $this->page->add_to_page('notifications',
                $this->renderer->notification(get_string('alreadyfilled', 'smartquest', $msgstring),
                \core\output\notification::NOTIFY_ERROR));
        } else {
            ...
            $viewform = data_submitted($CFG->wwwroot."/mod/smartquest/complete.php");
            ...
            if (data_submitted() && confirm_sesskey() && isset($viewform->submit) && isset($viewform->submittype) &&
                ...
                $aboutuser = isset($viewform->aboutuserid) ? $viewform->aboutuserid : null;
                $this->rid = $this->response_insert($this->survey->id, $viewform->sec, $viewform->rid, $quser, false, $aboutuser);
                ...
                smartquest_record_submission($this, $USER->id, $rid);

                if ($this->grade != 0) {
                    $smartquest = new stdClass();
                    $smartquest->id = $this->id;
                    $smartquest->name = $this->name;
                    $smartquest->grade = $this->grade;
                    $smartquest->cmidnumber = $this->cm->idnumber;
                    $smartquest->courseid = $this->course->id;
                    smartquest_update_grades($smartquest, $quser);
                }
                ...
                $params = array(
                                ...
                                'anonymous' => $anonymous,
                                'other' => array('smartquestid' => $smartquest->id)
                );
                $event = \mod_smartquest\event\attempt_submitted::create($params);
                $event->trigger();
    ...
    *
    */
    public function view_response($rid, $referer= '', $blanksmartquest = false, $resps = '', $compare = false,
        ...
        if ($referer != 'print') {
            ...
            if ($this->survey->feedbacknotes) {
                $text = file_rewrite_pluginfile_urls($this->survey->feedbacknotes, 'pluginfile.php',
                    $this->context->id, 'mod_smartquest', 'feedbacknotes', $this->survey->id);
    ...
    */
    public function view_all_responses($resps) {
        ...
        if ($resps) {
            ...
            foreach ($resps as $resp) {
                ...
                $aboutusertext[$resp->id] = '';

                if (($this->rtype == STUDEFFECT || $this->rtype == GUIDELINE) && isset($resp->aboutuserid)) {
                    if ($aboutuser = core_user::get_user($resp->aboutuserid)) {
                        $aboutusertext[$resp->id] = get_string('aboutuser', 'smartquest', $aboutuser);
                    }
                }
            }
            $i = 0;
                    ...
                    foreach ($data as $respid => $respdata) {
                        ...
                        if ($hasresp) {
                            $allrespdata[$i][] = [
                                ...
                                'respdata' => $respdata,
                                'respaboutuserid' => htmlspecialchars_decode($aboutusertext[$respid])
                            ];
                        }
                    }
                }
            ...
            $this->page->add_to_page('responses', $this->renderer->all_response_output(get_string('noresponses', 'smartquest')));
        }
    ...
    public function user_can_take($userid) {
        ...
        if (!$this->is_active() || !$this->user_is_eligible($userid)) {
            return false;
        } else if ($this->qtype == SMARTQUESTUNLIMITED) {
            return true;
        } else if ($userid > 0) {
    ...
    public function is_complete_definition() {
        $complete = true;

        if ($this->rtype == 1 && !$this->sapevent_id || ($this->rtype == 1 && $this->role_id && !$this->user_id)) {
            $complete = false;
        } else if ($this->rtype == 2 && (!$this->role_id || !$this->user_id)) {
            $complete = false;
        }
    ...
    public function get_rtype_text($aboutuserid = 0) {
        global $DB;

        $rtypetext = '';
        switch ($this->rtype) {
            case COURSE:
                $rtypetext = get_string('rtypecourse', 'smartquest');
                break;
            case SAPEVENT:
                $data = local_global_settings_get_sapevent($this->sapevent_id);
                $rtypetext = get_string('rtypesapevent', 'smartquest', $data);

                if (isset($this->role_id) && $this->role_id) {
                    if ($role = $DB->get_record('role', ['id' => $this->role_id])) {
                        $data->rolename = role_fix_names([$role], null, ROLENAME_ORIGINAL, true)[0];
                    } else {
                        $data->rolename = '';
                    }
                    if ($user = core_user::get_user($this->user_id)) {
                        $data->userfullname = $user ? $user->firstname . ' ' . $user->lastname : '';
                    } else {
                        $data->userfullname = '';
                    } 
                    $rtypetext = get_string('rtypesapeventwithrole', 'smartquest', $data);
                }
                break;
            case ROLETYPE:
                $data = new stdclass();
                ...
                $rtypetext = get_string('rtyperole', 'smartquest', $data);
                break;
            case STUDEFFECT:
            ...
            case GUIDELINE:
                ...
                if ($aboutuserid) {
                    if ($aboutuser = core_user::get_user($aboutuserid)) {
                        $rtypetext = get_string('rtypeguidelinewithuser', 'smartquest', $aboutuser);
                    }
                }
                break;
            case ROLEEFFECT:
                $rtypetext = get_string('rtyperoleeffect', 'smartquest');
                break;
        }
    ...
    public function user_has_saved_response($userid) {
        global $DB;
        //if anonimus return no responses
//Tamard
        //$anonymous = $DB->get_records('smartquest_anonymcourses', ['anonymcourse' => $this->course->id]);
        //if ($anonymous) {
         //   return;
        //}
        return $DB->record_exists('smartquest_response',
    ...
    public function user_time_for_new_attempt($userid) {
        ...
        if (!($attempts = $DB->get_records('smartquest_attempts', $params, 'timemodified DESC'))) {
            return true;
        }
    ...
    public function can_view_response($rid) {
        ...
        if (!empty($rid)) {
            $response = $DB->get_record('smartquest_response', array('id' => $rid));
            ...
            if ($this->capabilities->readallresponses &&
                ($this->resp_view == SMARTQUEST_STUDENTVIEWRESPONSES_ALWAYS ||
                 ($this->resp_view == SMARTQUEST_STUDENTVIEWRESPONSES_WHENCLOSED && $this->is_closed()) ||
                 ($this->resp_view == SMARTQUEST_STUDENTVIEWRESPONSES_WHENANSWERED  && !$this->user_can_take($USER->id)))) {
                return true;
            }
    ...
    public function can_view_all_responses($usernumresp = null) {
        ...
        if ($owner = $DB->get_field('smartquest_survey', 'courseid', ['id' => $this->sid])) {
        ...
        return (( // Teacher or non-editing teacher (if can view all groups).
                 (($canviewallgroups && $this->capabilities->readallresponseanytime)||
                ...
                ($this->capabilities->readallresponses && ($numresp > 0) && $canviewgroups &&
                 ...
                 ($this->resp_view == SMARTQUEST_STUDENTVIEWRESPONSES_ALWAYS ||
                  ($this->resp_view == SMARTQUEST_STUDENTVIEWRESPONSES_WHENCLOSED && $this->is_closed()) ||
                  ($this->resp_view == SMARTQUEST_STUDENTVIEWRESPONSES_WHENANSWERED && ($usernumresp > 0))) &&
    ...
    public function count_submissions($userid=false) {
        ...
        if (!$userid) {
            ...
            return $DB->count_records('smartquest_response', array('survey_id' => $this->sid, 'complete' => 'y'));
        } else {
            return $DB->count_records('smartquest_response', array('survey_id' => $this->sid, 'userid' => $userid,
                                      'complete' => 'y'));
        }
    }
    ...
    /**
     * Check if current smartquest has dependencies set and any question has dependencies.
     *
    ...
    public function print_survey($userid=false, $quser) {
        ...
        if (!isset($formdata->aboutuserid) || !$formdata->aboutuserid) {
            $formdata->aboutuserid = $this->get_response_aboutuserid($quser);
        }
        // If student saved a "resume" smartquest OR left a smartquest unfinished
        ...
        $msg = '';
        $action = $CFG->wwwroot.'/mod/smartquest/complete.php?id=' . $this->cm->id;
        ...
        if (!empty($formdata->resume) && ($this->resume)) {
            ...
            $formdata->rid = $this->response_insert($this->survey->id, $formdata->sec, $formdata->rid, $quser, $resume = true, $formdata->aboutuserid);
            $this->response_goto_saved($action);
            return;
        }

        // Save each section 's $formdata somewhere in case user returns to that page when navigating the smartquest.
        if (!empty($formdata->next)) {

            $this->response_delete($formdata->rid, $formdata->sec);
            $formdata->rid = $this->response_insert($this->survey->id, $formdata->sec, $formdata->rid, $quser, false, $formdata->aboutuserid);
            $msg = $this->response_check_format($formdata->sec, $formdata);
            if ( $msg ) {
                $formdata->next = '';
            } else {
                ...
                if ($this->has_dependencies()) {
                    while (!$this->eligible_questions_on_page($formdata->sec, $formdata->rid)) {
                        $this->response_delete($formdata->rid, $formdata->sec);
                        $formdata->sec++;
                        // We have reached the end of smartquest on a page without any question left.
                        if ($formdata->sec > $numsections) {
                            $SESSION->smartquest->end = true; // End of smartquest reached on a no questions page.
                            break;
                        }
                    }
                }
        ...
        if (!empty($formdata->prev)) { 
            ...
            // unlock smartquest->end to allow navigate back to previous page.
            if (isset($SESSION->smartquest->end) && ($SESSION->smartquest->end == true)) {
                $SESSION->smartquest->end = false;
                $formdata->sec--;
            }
        ...
        if (isset($this->questions) && $numsections) { // Sanity check.
            ...
            $controlbuttons = [];
            if ($formdata->sec > 1) {
                $controlbuttons['prev'] = ['type' => 'submit', 'value' => '<< '.get_string('previouspage', 'smartquest')];
            }
            if ($this->resume) {
                $controlbuttons['resume'] = ['type' => 'submit', 'value' => get_string('save', 'smartquest')];
            }
            ...
            if ($formdata->sec == $numsections) {
                ...
                $controlbuttons['submit'] = ['type' => 'submit', 'value' => get_string('submitsurvey', 'smartquest')];
            } else {
                $controlbuttons['next'] = ['type' => 'submit', 'value' => get_string('nextpage', 'smartquest').' >>'];
            }
            ...
            $this->page->add_to_page('controlbuttons',
                $this->renderer->complete_controlbuttons(get_string('noneinuse', 'smartquest')));
        }
    ...
    private function survey_render($section = 1, $message = '', &$formdata) {
        ...
        if ($section > $numsections) {
            ...
            $this->page->add_to_page('notifications',
                $this->renderer->notification(get_string('finished', 'smartquest'), \core\output\notification::NOTIFY_WARNING));
        ...
        if ($section == 1 && ($this->rtype == STUDEFFECT || $this->rtype == GUIDELINE)) {
            $usereffectdata = [
                'qelements' => [
                    'choice' => [
                        'id' => 'aboutuserid',
            ...
            $this->page->add_to_page('questions',
                $this->renderer->render_from_template('mod_smartquest/question_drop', $usereffectdata));
        }

        $this->print_survey_start($message, $section, $numsections, $hasrequired, '', 1, $formdata->aboutuserid);
        //print_r($formdata);die;
        foreach ($this->questionsbysec[$section] as $question) {
            if ($question->type_id != QUESSECTIONTEXT) {
                $i++;
            }
            // Need smartquest id to get the smartquest object in sectiontext (Label) question class.
            $formdata->smartquest_id = $this->id;
            $this->page->add_to_page('questions',
    ...
    private function print_survey_start($message, $section, $numsections, $hasrequired, $rid='', $blanksmartquest=false, $aboutuserid = 0) {
        ...
        if ($rid) {
            $courseid = $this->course->id;
            if ($resp = $DB->get_record('smartquest_response', array('id' => $rid)) ) {
                if ($this->respondenttype == 'fullname') {
                    $userid = $resp->userid;
                    // Display name of group(s) that student belongs to... if smartquest is set to Groups separate or visible.
                    ...
                    $event = \mod_smartquest\event\response_viewed::create($params);
                    $event->trigger();
                }
            }
        }
        $ruser = '';
        if ($resp && !$blanksmartquest) {
            if ($userid) {
            ...
            if ($this->respondenttype == 'anonymous') {
                ...
                if ($resp->submitted) {
                    $timesubmitted = '&nbsp;'.get_string('submitted', 'smartquest').'&nbsp;'.userdate($resp->submitted);
                }
            }
        }
        if ($ruser) {
            ...
            if ($this->survey->realm == 'public') {
                ...
                $sql = 'SELECT q.id, q.course, c.fullname '.
                       'FROM {smartquest} q, {smartquest_attempts} qa, {course} c '.
        ...
        if ($this->capabilities->printblank && $blanksmartquest && $section == 1) {
            ...
            $url = '/mod/smartquest/print.php?qid='.$this->id.'&amp;rid=0&amp;'.'courseid='.$this->course->id.'&amp;sec=1';
        ...
        if ($section == 1) {
            ...
            if ($this->survey->info) {
                $infotext = file_rewrite_pluginfile_urls($this->survey->info, 'pluginfile.php',
                                $this->context->id, 'mod_smartquest', 'info', $this->survey->id);
                $this->page->add_to_page('addinfo', $infotext);
            }
        }

        $aboutuserid = isset($resp->aboutuserid) ? $resp->aboutuserid : $aboutuserid;
      
        $rtypetext = $this->get_rtype_text($aboutuserid);
        $this->page->add_to_page('rtypetext', $rtypetext);
    ...
    private function print_survey_end($section, $numsections) {
        ...
        if ($numsections > 1) {
            ...
            $this->page->add_to_page('pageinfo',
                $this->renderer->container(get_string('pageof', 'smartquest', $a).'&nbsp;&nbsp;', 'surveyPage'));
        }
    }

    // Blanksmartquest : if we are printing a blank smartquest.
    public function survey_print_render($message = '', $referer='', $courseid, $rid=0, $blanksmartquest=false) {
        global $DB, $CFG;
        ...
        if (!empty($rid)) {
            ...
            $this->view_response($rid, $referer, $blanksmartquest);
            return;
        }
        ...
        $action = $CFG->wwwroot.'/mod/smartquest/preview.php?id='.$this->cm->id;
        ...
        if (data_submitted()) {
            ...
            foreach ($this->questionsbysec as $section) {
                ...
                if ($errormessage) {
                    if ($numsections > 1) {
                        $pageerror = get_string('page', 'smartquest').' '.$s.' : ';
                    }
                    $this->page->add_to_page('notifications',
        ...
        if ($errors == 0) {
            $this->page->add_to_page('message',
                $this->renderer->notification(get_string('submitpreviewcorrect', 'smartquest'),
        ...
        foreach ($this->questionsbysec as $section) {
            $output = '';
            if ($numsections > 1) {
                $output .= $this->renderer->print_preview_pagenumber(get_string('page', 'smartquest').' '.$page);
                $page++;
            }
        ...
        // End of questions.
        if ($referer == 'preview' && !$blanksmartquest) {
            $url = $CFG->wwwroot.'/mod/smartquest/preview.php?id='.$this->cm->id;
            $this->page->add_to_page('formend',
                $this->renderer->print_preview_formend($url, get_string('submitpreview', 'smartquest'), get_string('reset')));
        }
        return;
    }
    ...
    public function survey_update($sdata) {
        ...
        if (empty($this->survey->id)) {
            ...
            $fields = array('name', 'realm', 'title', 'subtitle', 'email', 'theme', 'thanks_page', 'thank_head',
                            'thank_body', 'feedbacknotes', 'info', 'feedbacksections', 'feedbackscores', 'chart_type', 'anonymoustemplate');
            ...
            $this->survey->id = $DB->insert_record('smartquest_survey', $record);
            $this->add_survey($this->survey->id);

            if (!$this->survey->id) {
                $errstr = get_string('errnewname', 'smartquest') .' [ :  ]'; // TODO: notused!
                return(false);
            }
        } else {
            ...
            $fields = array('name', 'realm', 'title', 'subtitle', 'email', 'theme', 'thanks_page',
                    'thank_head', 'thank_body', 'feedbacknotes', 'info', 'feedbacksections', 'feedbackscores', 'chart_type', 'anonymoustemplate');
            $name = $DB->get_field('smartquest_survey', 'name', array('id' => $this->survey->id));

            // Trying to change survey name.
            if (trim($name) != trim(stripslashes($sdata->name))) {  // $sdata will already have slashes added to it.
                $count = $DB->count_records('smartquest_survey', array('name' => $sdata->name));
                if ($count != 0) {
                    $errstr = get_string('errnewname', 'smartquest');  // TODO: notused!
                    return(false);
                }
            ...
            $result = $DB->update_record('smartquest_survey', $surveyrecord);
            if (!$result) {
                $errstr = get_string('warning', 'smartquest').' [ :  ]';  // TODO: notused!
                return(false);
            }
        }
    ...
    public function survey_copy($owner) {
        ...
        $i = 0;
        $name = $survey->name;
        while ($DB->count_records('smartquest_survey', array('name' => $name)) > 0) {
            $name = $survey->name.(++$i);
        }
        if ($i) {
            $survey->name .= $i;
        ...
        // Create new survey.
        if (!($newsid = $DB->insert_record('smartquest_survey', $survey))) {
            return(false);
        }
        ...
        foreach ($this->questions as $question) {
            ...
            $oldid = $question->id;
            if ($question->sourceid == 0) {
                $question->sourceid = $oldid;
            }
            unset($question->id);
            ...
            foreach ($question->choices as $key => $choice) {
                ...
                if (!$newcid = $DB->insert_record('smartquest_quest_choice', $choice)) {
                    return(false);
                }
        ...
        $dependquestions = $DB->get_records('smartquest_dependency', ['surveyid' => $this->survey->id], 'questionid');
        foreach ($dependquestions as $dquestion) {
            ...
            $DB->insert_record('smartquest_dependency', $record);
        }

        // Replicate any feedback data.
        // TODO: Need to handle image attachments (same for other copies above).
        $fbsections = $DB->get_records('smartquest_fb_sections', ['survey_id' => $this->survey->id], 'id');
        foreach ($fbsections as $fbsid => $fbsection) {
            ...
            unset($fbsection->id);
            $newfbsid = $DB->insert_record('smartquest_fb_sections', $fbsection);
            $feedbackrecs = $DB->get_records('smartquest_feedback', ['section_id' => $fbsid], 'id');
            foreach ($feedbackrecs as $feedbackrec) {
                $feedbackrec->section_id = $newfbsid;
                unset($feedbackrec->id);
                $DB->insert_record('smartquest_feedback', $feedbackrec);
            }
        }
    ...
    private function response_check_format($section, $formdata, $checkmissing = true, $checkwrongformat = true) {
        ...
        foreach ($this->questionsbysec[$section] as $question) {
            ...
            if (!$question->response_complete($formdata)) {
                $missing++;
                $strmissing .= get_string('num', 'smartquest').$qnum.'. ';
            }
            if (!$question->response_valid($formdata)) {
                $wrongformat++;
                $strwrongformat .= get_string('num', 'smartquest').$qnum.'. ';
            }
        }
        $message = '';
        $nonumbering = false;
        ...
        if ($checkmissing && $missing) {
            ...
            if ($missing == 1) {
                $message = get_string('missingquestion', 'smartquest').$strmissing;
            } else {
                $message = get_string('missingquestions', 'smartquest').$strmissing;
            }
            if ($wrongformat) {
                $message .= '<br />';
            }
        ...
        if ($checkwrongformat && $wrongformat) {
            if ($nonumbering) {
                $message .= get_string('wronganswers', 'smartquest');
            } else {
                if ($wrongformat == 1) {
                    $message .= get_string('wrongformat', 'smartquest').$strwrongformat;
                } else {
                    $message .= get_string('wrongformats', 'smartquest').$strwrongformat;
                }
            }
        }
        return ($message);
    ...
    private function response_delete($rid, $sec = null) {
        ...
        foreach (array('response_bool', 'resp_single', 'resp_multiple', 'response_rank', 'response_relrank', 'response_text',
                       'response_other', 'response_date') as $tbl) {
            $DB->delete_records_select('smartquest_'.$tbl, $select, $params);
        }
    }
    ...
    private function response_import_sec($rid, $sec, &$varr) {
        ...
        foreach ($vals as $id => $arr) {
            if (isset($arr[0]) && is_array($arr[0])) {
                ...
                $nid = substr($id, 0, strpos( $id, '_'));
                if (isset($arr[2]) && strstr($id, '_') && isset($this->questions[$nid]) && $this->questions[$nid]->type_id == 11) {
                    $varr->{'q'.$id} = $arr;
                } else {
    ...
    private function response_commit($rid) {
        ...
        return $DB->update_record('smartquest_response', $record);
    }
...
//Tamard        
//$anonymous = $DB->get_records('smartquest_anonymcourses', ['anonymcourse' => $this->course->id]);
        //if ($resume && $anonymous || $anonymous && !$rid) {//&& anonimus 
          //  return '';
        //}
        $rid = intval($rid);
        if ($rid != 0) {
            ...
            return ($DB->get_record_select('smartquest_response', $select, null, $fields) !== false) ? $rid : '';

        } else {
            // Find latest in progress rid.
            $select = 'survey_id = '.$this->sid.' AND complete = \'n\' AND userid = '.$userid;
            if ($records = $DB->get_records_select('smartquest_response', $select, null, 'submitted DESC',
                                              'id,survey_id', 0, 1)) {
                $rec = reset($records);
                return $rec->id;
            } else {
    ...
    private function get_response_aboutuserid($userid, $rid = 0) {
        global $DB, $COURSE;

        $rid = intval($rid);
        if ($rid != 0) {
            // Check for valid rid.
            $fields = 'id, userid, aboutuserid';
            //$select = 'id = '.$rid.' AND survey_id = '.$this->sid.' AND userid = '.$userid.' AND complete = \'n\'';
            $select = 'id = '.$rid.' AND survey_id = '.$this->sid.' AND userid = '.$userid;
            return ($DB->get_record_select('smartquest_response', $select, null, $fields) !== false) ? $rid : '';
            ...
            //**************/
            //$select = 'survey_id = '.$this->sid.' AND complete = \'n\' AND userid = '.$userid;
            $select = 'survey_id = '.$this->sid.'  AND userid = '.$userid;
            ...
            //if ($records = $DB->get_records_select('smartquest_response', $select, null, 'submitted DESC',
            //                                  'id,survey_id, aboutuserid', 0, 1)) {
            //    $rec = reset($records);
            //    print_r($rec);
                return array_values($users)[0]->id;
            // } else {
            //     return '';
            // }
        }
    ...
    private function response_select_max_sec($rid) {
        ...
        $max = $DB->count_records_select('smartquest_question', $select, $params) + 1;
    ...
    private function response_select_max_pos($rid) {
        ...
        foreach (array('response_bool', 'resp_single', 'resp_multiple', 'response_rank', 'response_relrank', 'response_text',
                       'response_other', 'response_date') as $tbl) {
            $sql = 'SELECT MAX(q.position) as num FROM {smartquest_'.$tbl.'} a, {smartquest_question} q '.
    ...
    private function send_submission_notifications($rid) {
        ...
        if ($notifyusers = $this->get_notifiable_users($USER->id)) {
            ...
            $info->submissionurl = $CFG->wwwroot.'/mod/smartquest/report.php?action=vresp&sid='.$this->survey->id.
                    '&rid='.$rid.'&instance='.$this->id;

            $info->postsubject = get_string('submissionnotificationsubject', 'smartquest');
            $info->posttext = get_string($langstringtext, 'smartquest', $info);
            $info->posthtml = '<p>' . get_string($langstringhtml, 'smartquest', $info) . '</p>';
    ...
    private function send_message($info, $eventtype) {
        ...
        $eventdata->modulename       = 'smartquest';
    ...
    protected function get_notifiable_users($userid) {
        ...
        $potentialusers = get_enrolled_users($this->context, 'mod/smartquest:submissionnotification',
    ...
    private function response_send_email($rid) {
        ...
        } else if ($record = $DB->get_record('smartquest_survey', ['id' => $this->survey->id])) {
            $email = $record->email;
        } else {
            $email = '';
        }
        ...
        $subject = get_string('surveyresponse', 'smartquest') .": $name [$rid]";
        $url = $CFG->wwwroot.'/mod/smartquest/report.php?action=vresp&amp;sid='.$this->survey->id.
                '&amp;rid='.$rid.'&amp;instance='.$this->id;

        // Html and plaintext body.
        $bodyhtml        = '<a href="'.$url.'">'.$url.'</a>'.$endhtml;
        $bodyplaintext   = $url.$endplaintext;
        $bodyhtml       .= get_string('surveyresponse', 'smartquest') .' "'.$name.'"'.$endhtml;
        $bodyplaintext  .= get_string('surveyresponse', 'smartquest') .' "'.$name.'"'.$endplaintext;
    ...
    public function response_insert($sid, $section, $rid, $userid, $resume=false, $aboutuserid = 0) {
        global $DB;
        $record = new stdClass();
        $record->submitted = time();
        //$record->complete = 'y';
        if ($aboutuserid) {
            $record->aboutuserid = $aboutuserid;
        }
        ...
        if (empty($rid)) {
            ...
            $rid = $DB->insert_record('smartquest_response', $record);
        } else {
            $record->id = $rid;
            $DB->update_record('smartquest_response', $record);
        }
        if ($resume) {
            ...
            $params = array(
                            ...
                            'relateduserid' => $userid,
                            'anonymous' => $anonymous,
                            'other' => array('smartquestid' => $this->id)
            );
            $event = \mod_smartquest\event\attempt_saved::create($params);
            $event->trigger();
        }
        ...
        if (!empty($this->questionsbysec[$section])) {
            foreach ($this->questionsbysec[$section] as $question) {
                ...
                } else if ($question->responsetable == 'response_relrank'){
                    // $val = [];
                    $val = optional_param_array('q'.$question->id, '', PARAM_RAW);
                    // $val[1] = optional_param_array('nrq'.$question->id, '', PARAM_RAW);
                } else {
    ...
    private function response_select($rid, $col = null, $csvexport = false, $choicecodes=0, $choicetext=1) {
        ...
        $values = \mod_smartquest\response\boolean::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_single (radio button or dropdown).
        $values += \mod_smartquest\response\single::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_multiple.
        $values += \mod_smartquest\response\multiple::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_rank.
        $values += \mod_smartquest\response\rank::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_relevantrank.
        $values += \mod_smartquest\response\relevantrank::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_text.
        $values += \mod_smartquest\response\text::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_date.
        $values += \mod_smartquest\response\date::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);
    ...
    private function response_goto_thankyou() {
        ...
        if ($result = $DB->get_record_select('smartquest_survey', $select, null, $fields)) {
        ...
        if (empty($thankhead)) {
            $thankhead = get_string('thank_head', 'smartquest');
        }
        ...
        $this->page->add_to_page('addinfo',
            ...
            $this->context->id, 'mod_smartquest', 'thankbody', $this->survey->id), FORMAT_HTML, ['noclean' => true]));
    ...
    private function response_goto_saved($url) {
        global $CFG;
        $resumesurvey = get_string('resumesurvey', 'smartquest');
        $savedprogress = get_string('savedprogress', 'smartquest', '<strong>'.$resumesurvey.'</strong>');
    ...
    public function survey_results_navbar_alpha($currrid, $currentgroupid, $cm, $byresponse) {
        ...
        // Is this smartquest set to fullname or anonymous?
        ...
        if ($isfullname) {
            ...
            $sql = 'SELECT R.id AS responseid, R.submitted AS submitted, R.userid, R.aboutuserid aboutuserid, U.username AS username,
            ...
            $sql = 'SELECT R.id AS responseid, R.submitted, R.aboutuserid aboutuserid
                   FROM {smartquest_response} R
                   WHERE R.survey_id = ?
                   AND complete = ?
        ...
        foreach ($responses as $response) {
            ...
            if (($this->rtype == STUDEFFECT || $this->rtype == GUIDELINE) && isset($response->aboutuserid)) {
                if ($aboutuser = core_user::get_user($response->aboutuserid)) {
                    array_push($ridsaboutuserid, get_string('aboutuser', 'smartquest', $aboutuser));
                } else {
                    array_push($ridsaboutuserid, '');
                }
            } else {
                array_push($ridsaboutuserid, '');
            }
        ...
        $url = $CFG->wwwroot.'/mod/smartquest/report.php?action=vresp&group='.$currentgroupid.'&individualresponse=1';
        if (!$byresponse) {     // Display navbar.
            ...
            $url = $CFG->wwwroot.'/mod/smartquest/report.php?action=vresp&byresponse=1&group='.$currentgroupid;
            // Display navbar.
            $navbar->listlink = $url;

            // Display a "print this response" icon here in prevision of total removal of tabs in version 2.6.
            $linkname = '&nbsp;'.get_string('print', 'smartquest');
            $url = '/mod/smartquest/print.php?qid='.$this->id.'&rid='.$currrid.
            '&courseid='.$this->course->id.'&sec=1';
            $title = get_string('printtooltip', 'smartquest');
            ...
            for ($i = 0; $i < $total; $i++) {
                if ($isfullname) {
                    ...
                    $resparr[] = '<a title = "'.$responsedate.'" href="'.$url.'&amp;rid='.
                        $rids[$i].'&amp;individualresponse=1" >' . $ridsuserfullname[$i] . ' ' . $ridsaboutuserid[$i] . " ".  $responsedate . '</a> ';
                } else {
                    $responsedate = '';
                    $resparr[] = '<a title = "'.$responsedate.'" href="'.$url.'&amp;rid='.
                        $rids[$i].'&amp;individualresponse=1" >'.
                        get_string('response', 'smartquest').($i + 1).'</a> ';
                }
            }
    ...
    public function survey_results($precision = 1, $showtotals = 1, $qid = '', $cids = '', $rid = '',
        ...
        $SESSION->smartquest->noresponses = false;
        if (empty($precision)) {
            $precision  = 1;
        }
        if ($showtotals === '') {
        ...
        $haschoices = array();
        $responsetable = array();
        if (!($types = $DB->get_records('smartquest_question_type', array(), 'typeid', 'typeid, has_choices, response_table'))) {
            $errmsg = sprintf('%s [ %s: question_type ]',
                    get_string('errortable', 'smartquest'), 'Table');
            return($errmsg);
        }
        ...
        if (empty($this->survey)) {
            $errmsg = get_string('erroropening', 'smartquest') ." [ ID:{$this->sid} R:";
            return($errmsg);
        }

        if (empty($this->questions)) {
            $errmsg = get_string('erroropening', 'smartquest') .' '. 'No questions found.';
            return($errmsg);
        }
        ...
        if (!empty($rid)) {
            ...
            if (!($rows = $DB->get_records_sql($sql))) {
                $this->page->add_to_page('respondentinfo',
                    $this->renderer->notification(get_string('noresponses', 'smartquest'),
                    \core\output\notification::NOTIFY_ERROR));
                $SESSION->smartquest->noresponses = true;
                return;
            }
            $total = count($rows);
            $this->page->add_to_page('respondentinfo',
                ' '.get_string('responses', 'smartquest').': <strong>'.$total.'</strong>');
            if (empty($rows)) {
                $errmsg = get_string('erroropening', 'smartquest') .' '. get_string('noresponsedata', 'smartquest');
                    return($errmsg);
            }
        ...
        foreach ($this->questions as $question) {
            ...
            $this->page->add_to_page('responses',
                ...
                $question->context->id, 'mod_smartquest', 'question', $question->id),
    ...
    protected function choice_types() {
        return [QUESRADIO, QUESDROP, QUESCHECK, QUESRATE, QUESRELEVANTRATE];
    }

    /**
     * Return all the fields to be used for users in smartquest sql.
     *
     * @author: Guy Thomas
     * @return array|string
     */
    ...
    protected function get_survey_all_responses($rid = '', $userid = '', $groupid = false) {
        ...
        foreach ($uniquetypes as $type) {
            $question = \mod_smartquest\question\base::question_builder($type);
    ...
    protected function process_csv_row(array &$row,
        ...
        if ($config === null) {
            $config = get_config('smartquest', 'downloadoptions');
        }
        ...
        // Moodle:
        // Get the course name that this smartquest belongs to.
        if ($this->survey->realm != 'public') {
            $courseid = $this->course->id;
        ...
        if ($isanonymous) {
            $fullname = get_string('anonymous', 'smartquest');
            $username = '';
            $uid = '';
        } else {
            $uid = $user->id;
        ...
        if (in_array('submitted', $options)) {
            ...
            $submitted = date(get_string('strfdateformatcsv', 'smartquest'), $resprow->submitted);
        ...
        if ($this->rtype == STUDEFFECT || $this->rtype == GUIDELINE) {
            $aboutuser = core_user::get_user($resprow->aboutuserid);
            array_push($positioned, $aboutuser ? $aboutuser->firstname . ' ' . $aboutuser->lastname : ''); 
            if (in_array('institution', $options)) {
                array_push($positioned, $aboutuser ? $aboutuser->institution : ''); 
            }
            if (in_array('department', $options)) {
                array_push($positioned, $aboutuser ? $aboutuser->department : ''); 
            }
        } else {
            if (in_array('institution', $options)) {
    ...
    */
    public function generate_csv($rid='', $userid='', $choicecodes=1, $choicetext=0, $currentgroupid) {
        ...
        $output = array();
        $stringother = get_string('other', 'smartquest');
        ...
        foreach ($options as $option) {
            if ($this->rtype != STUDEFFECT && $this->rtype != GUIDELINE && $option == 'aboutusercsv') {
                continue;
            }

            if (in_array($option, array('response', 'submitted', 'id', 'aboutusercsv'))) {
                $columns[] = get_string($option, 'smartquest');
                $types[] = 0;
            } else {
        ...
        $idtocsvmap = array(
            ...
            '1',    // 9: date -> string.
            '0',    // 10: numeric -> number.
            '0'     // 11: relevant rate -> number.
        );

        if (!$survey = $DB->get_record('smartquest_survey', array('id' => $this->survey->id))) {
...
//if (count(array_intersect($choicetypes, $uniquetypes) > 0 )) {
        if (count(array_intersect($choicetypes, $uniquetypes)) > 0 ) {
            ...
            $choicesql = "
                SELECT DISTINCT c.id as cid, q.id as qid, q.precise AS precise, q.name, c.content
                  FROM {smartquest_question} q
                  JOIN {smartquest_quest_choice} c ON question_id = q.id
                ...
                switch ($type) {
                    ...
                    case QUESCHECK: // Multiple.
                        ...
                        foreach ($choices as $choice) {
                            $content = $choice->content;
                            $modality = '';
                            $contents = smartquest_choice_values($content);
                            if ($contents->modname) {
                        ...
                        foreach ($choices as $choice) {
                            ...
                            if (preg_match("/^[0-9]{1,3}=/", $content, $ndd)) {
                                ...
                                if ($osgood) {
                                    ...
                                    $contents = smartquest_choice_values($contentleft);
                                    if ($contents->title) {
                                        $contentleft = $contents->title;
                                    }
                                    $contents = smartquest_choice_values($contentright);
                                    if ($contents->title) {
            ...
            if ($qtype === QUESRATE || $qtype === QUESCHECK || $qtype === QUESRELEVANTRATE) {
                ...
                if ($questionobj->has_choices()) {
                    ...
                    if (preg_match('/^!other/', $content)) {
                        ...
                        $responsetxt = get_string('other', 'smartquest');
        ...
        if (is_array($questions)) {
            ...
            foreach ($questions as $question) {
                ...
                if ($question->id == $movequestion->id) {
                    ...
                    $DB->update_record("smartquest_question", $movequestion);
                    continue;
                }
                $question->position = $index;
                $DB->update_record("smartquest_question", $question);
                $index++;
            }
            return true;
        }
    ...
    public function response_analysis ($rid, $resps, $compare, $isgroupmember, $allresponses, $currentgroupid,
        ...
        require_once($CFG->dirroot.'/mod/smartquest/drawchart.php');

        // Find if there are any feedbacks in this smartquest.
        $sql = "SELECT * FROM {smartquest_fb_sections} WHERE survey_id = ? AND section IS NOT NULL";
        ...
        if ($resp = $DB->get_record('smartquest_response', ['id' => $rid]) ) {
            $userid = $resp->userid;
        ...
        if ($this->survey->feedbackscores) {
            ...
            $table->wrap = [];
            if ($compare) {
                $table->head = [get_string('feedbacksection', 'smartquest'), $ruser, $groupname];
            } else {
                $table->head = [get_string('feedbacksection', 'smartquest'), $groupname];
            }
        }
        ...
        // Calculate max score per question in smartquest.
        $qmax = [];
        $maxtotalscore = 0;
        foreach ($this->questions as $question) {
            $qid = $question->id;
        ...
        if ($this->survey->feedbacksections == 1) {
            ...
            $feedbacks = $DB->get_records('smartquest_feedback', array('section_id' => $sectionid));
            $labels = array();
            ...
            $feedback = $DB->get_record_select('smartquest_feedback',
            ...
            $sectionheading = file_rewrite_pluginfile_urls($sectionheading, 'pluginfile.php',
                            $this->context->id, 'mod_smartquest', 'sectionheading', $sectionid);
            ...
            if (!empty($feedback->feedbacktext)) {
                ...
                $feedbacktext = file_rewrite_pluginfile_urls($feedback->feedbacktext, 'pluginfile.php',
                                $this->context->id, 'mod_smartquest', 'feedback', $feedback->id);
        ...
        for ($section = 1; $section <= $this->survey->feedbacksections; $section++) {
            ...
            if (!$allresponses) {
                ...
                $formatoptions->noclean = true;
                $sectionheading = file_rewrite_pluginfile_urls($sectionheading, 'pluginfile.php',
                                $this->context->id, 'mod_smartquest', 'sectionheading', $imageid);
