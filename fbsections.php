...
$url = new moodle_url('/mod/smartquest/fbsections.php');
if ($id !== 0) {
    $url->param('id', $id);
}
if ($sid) {
...
$questions = $smartquest->questions;
$sid = $smartquest->survey->id;
$viewform = data_submitted($CFG->wwwroot."/mod/smartquest/fbsections.php");
...
if (data_submitted()) {
    ...
    if ($c < $feedbacksections) {
        ...
        $errormsg = get_string('sectionsnotset', 'smartquest', $sectionsnotset);
        $vf = $submittedvf;
    } else {
        for ($section = 1; $section <= $feedbacksections; $section++) {
        ...
        $sections = $DB->get_records('smartquest_fb_sections',
            array('survey_id' => $smartquest->survey->id), 'section DESC');
        ...
        foreach ($sections as $section) {
            if ($section->section > $feedbacksections) {
                ...
                $DB->delete_records('smartquest_fb_sections', array('survey_id' => $sid, 'section' => $section->section));
                // Delete associated feedback records.
                $DB->delete_records('smartquest_feedback', array('section_id' => $section->section));
            }
        }
        ...
        for ($section = 1; $section <= $feedbacksections; $section++) {
            if ($existsection = $DB->get_record('smartquest_fb_sections',
                ...
                $DB->set_field('smartquest_fb_sections', 'scorecalculation', serialize($scorecalculation[$section]),
                ...
                $feedbacksection->id = $DB->insert_record('smartquest_fb_sections', $feedbacksection);
            }
        }

        $currentsection = 1;
        $SESSION->smartquest->currentfbsection = 1;
        redirect ($CFG->wwwroot.'/mod/smartquest/fbsettings.php?id='.
            $smartquest->cm->id.'&currentsection='.$currentsection, '', 0);
    }
}
...
if (!isset($scorecalculationweights)) {
    $scorecalculationweights = [];
    if ($fbsections = $DB->get_records('smartquest_fb_sections', ['survey_id' => $sid], 'section ASC')) {
        for ($section = 1; $section <= $feedbacksections; $section++) {
...
$PAGE->set_title(get_string('feedbackeditingsections', 'smartquest'));
$PAGE->set_heading(format_string($course->fullname));
$PAGE->navbar->add(get_string('feedbackeditingsections', 'smartquest'));
...
$smartquest->add_page(new \mod_smartquest\output\fbsectionspage());

$feedbacksections = $smartquest->survey->feedbacksections + 1;

if ($errormsg != '') {
    $smartquest->page->add_to_page('notifications', $smartquest->renderer->notification($errormsg));
}
$n = 0;
...
$smartquest->page->add_to_page('formarea', $smartquest->renderer->box_start());

$smartquest->page->add_to_page('formarea', $smartquest->renderer->help_icon('feedbacksectionsselect', 'smartquest'));
$smartquest->page->add_to_page('formarea', '<b>Sections:</b><br /><br />');
...
foreach ($smartquest->questions as $question) {
    ...
    if ($question->supports_feedback()) {
        ...
        if ($qname == '') {
            $cannotuse = true;
            $strcannotuse = get_string('missingname', 'smartquest', $qn);
        }
        if (!$question->required()) {
            $cannotuse = true;
            if ($qname == '') {
                $strcannotuse = get_string('missingnameandrequired', 'smartquest', $qn);
            } else {
                $strcannotuse = get_string('missingrequired', 'smartquest', $qn);
            }
        }

        if (!$cannotuse) {
            if ($question->valid_feedback()) {
                $smartquest->page->add_to_page('formarea', '<div id="group_'.$qid.'">');
                $emptyisglobalfeedback = ($smartquest->survey->feedbacksections == 1) && empty($questionsinsections);
                $smartquest->page->add_to_page('formarea', '<div style="margin-bottom:5px;">[' . $qname . ']</div>');
                ...
                $smartquest->page->add_to_page('formarea',
                    $smartquest->renderer->question_output($question, $formdata, [], $n, true));
            ...
            $smartquest->page->add_to_page('formarea', '<div class="notifyproblem">');
            $smartquest->page->add_to_page('formarea', $strcannotuse);
            $smartquest->page->add_to_page('formarea', '</div>');
            $smartquest->page->add_to_page('formarea', '<div class="qn-question">' . $question->content . '</div>');
        }
    }
}
...
$strfunc .= "\n</script>\n";
$smartquest->page->add_to_page('formarea', $strfunc);

// Submit/Cancel buttons.
$url = $CFG->wwwroot.'/mod/smartquest/view.php?id='.$cm->id;
$smartquest->page->add_to_page('formarea', '<div><input type="submit" name="savesettings" value="' .
    get_string('feedbackeditmessages', 'smartquest').'" class="btn btn-primary" /></div>');
