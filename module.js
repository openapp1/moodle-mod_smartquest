 ...
 * smartquest with conditional branching.
 */
...
function other_relevantrate_uncheck(name, value) {
    var col_name = name.substr(0, name.indexOf("_"));
    var inputbuttons = document.getElementsByTagName("input");
    for (var i = 0; i <= inputbuttons.length - 1; i++) {
        var button = inputbuttons[i];
...
function not_relevant_change(name, value) {
    // $('input[type="text"][nama="'+name+'"]').css('display', 'block');
    var inputs = document.getElementsByName('nr'+name);
...
M.mod_smartquest = M.mod_smartquest || {};

/* exported Y */
/* exported e */
M.mod_smartquest.init_attempt_form = function(Y) {
    M.core_formchangechecker.init({formid: 'phpesp_response'});
};

M.mod_smartquest.init_sendmessage = function(Y) {
