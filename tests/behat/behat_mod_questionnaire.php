...
/**
 * Steps definitions related with the smartquest activity.
 *
...
;
/**
 * Smsrtquest-related steps definitions.
 *
 * @package    mod_smartquest
 * @category   test
 * @copyright  2016 Mike Churchward - The POET Group
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_smartquest extends behat_base {

    /**
     * Adds a question to the smartquest with the provided data.
     *
    ...
    /**
     * Adds a questions and responses to the smartquest with the provided name.
     *
     * @Given /^"([^"]*)" has questions and responses$/
     *
     * @param string $smartquestname The name of an existing smartquest.
     */
    public function has_questions_and_responses($smartquestname) {
        global $DB;

        if (!$smartquest = $DB->get_record('smartquest', array('name' => $smartquestname), 'id,sid')) {
            throw new ExpectationException('Invalid smartquest name specified.', $this->getSession());
        }

        if (!$DB->record_exists('smartquest_survey', array('id' => $smartquest->sid))) {
            throw new ExpectationException('Smsrtquest survey does not exist.', $this->getSession());
        }

        $this->add_question_data($smartquest->sid);
        $this->add_response_data($smartquest->id, $smartquest->sid);
    }

    /**
     * Adds a question data to the given survey id.
     *
     * @param int $sid The id field of an existing smartquest_survey record.
     * @return null
     */
    private function add_question_data($sid) {
        ...
        $this->add_data($questiondata, 'smartquest_question', 'questionmap');
        $this->add_data($choicedata, 'smartquest_quest_choice', 'choicemap', array('questionmap' => 'question_id'));
    }

    /**
     * Adds response data to the given smartquest and survey id.
     *
     * @param int $qid The id field of an existing smartquest record.
     * @param int $sid The id field of an existing smartquest_survey record.
     * @return null
     */
    private function add_response_data($qid, $sid) {
        ...
        $this->add_data($responses, 'smartquest_response', 'responsemap');
        ...
        $this->add_data($attempts, 'smartquest_attempts', '', array('responsemap' => 'rid'));
        ...
        $this->add_data($responsebool, 'smartquest_response_bool', '',
        ...
        $this->add_data($responsedate, 'smartquest_response_date', '',
            array('responsemap' => 'response_id', 'questionmap' => 'question_id'));
        ...
        $this->add_data($responseother, 'smartquest_response_other', '',
        ...
        $this->add_data($responserank, 'smartquest_response_rank', '',
            array('responsemap' => 'response_id', 'questionmap' => 'question_id', 'choicemap' => 'choice_id'));
        ...
        $this->add_data($respmultiple, 'smartquest_resp_multiple', '',
            array('responsemap' => 'response_id', 'questionmap' => 'question_id', 'choicemap' => 'choice_id'));
        ...
        $this->add_data($respsingle, 'smartquest_resp_single', '',
            array('responsemap' => 'response_id', 'questionmap' => 'question_id', 'choicemap' => 'choice_id'));
    }
