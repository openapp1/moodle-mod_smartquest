@mod @mod_smartquest
Feature: In smartquest, personality tests can be constructed using feedback on specific question responses and questions can be
  assigned to multiple sections.
  In order to define a feedback smartquest
  As a teacher
  I must add the required question types and complete the feedback options with more than one section per question.

  @javascript
  Scenario: Create a smartquest with a with feeback question types and add more than one feedback section.
