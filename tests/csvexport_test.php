...
/**
 * Test performance of smartquest.
 * @author    Guy Thomas
...
/**
 * Performance test for smartquest module.
...
class mod_smartquest_csvexport_test extends advanced_testcase {
    ...
    public function test_csvexport() {
        ...
        $qdg = $dg->get_plugin_generator('mod_smartquest');
        $qdg->create_and_fully_populate(1, 5, 1, 1);

        // The following line simply.
        $smartquests = $qdg->smartquests();
        foreach ($smartquests as $smartquest) {
            list ($course, $cm) = get_course_and_cm_from_instance($smartquest->id, 'smartquest', $smartquest->course);
            $smartquestinst = new smartquest(0, $smartquest, $course, $cm);
            $newoutput = $this->get_csv_text($smartquestinst->generate_csv('', '', 0, 0, 0));
