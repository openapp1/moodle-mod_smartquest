...
use mod_smartquest\generator\question_response,
    mod_smartquest\generator\question_response_rank,
    mod_smartquest\question\base;

global $CFG;
require_once($CFG->dirroot.'/mod/smartquest/locallib.php');

class mod_smartquest_generator extends testing_module_generator {
    ...
    /**
     * Acessor for smartquests.
     *
     * @return array
     */
    public function smartquests() {
        return $this->smartquests;
    }

    /**
     * Create a smartquest activity.
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        $defaultsmartquestsettings = array(
            'qtype'                 => 0,
        ...
        foreach ($defaultsmartquestsettings as $name => $value) {
        ...
        $cm = get_coursemodule_from_instance('smartquest', $instance->id);
        $course = get_course($cm->course);
        $smartquest = new smartquest(0, $instance, $course, $cm, false);

        $this->smartquests[$instance->id] = $smartquest;

        return $smartquest;
    }

    /**
     * Create a survey instance with data from an existing smartquest object.
     * @param object $smartquest
     * @param array $options
     * @return int
     */
    public function create_content($smartquest, $record = array()) {
        global $DB;

        $survey = $DB->get_record('smartquest_survey', array('id' => $smartquest->sid), '*', MUST_EXIST);
        foreach ($record as $name => $value) {
            $survey->{$name} = $value;
        }
        return $smartquest->survey_update($survey);
    }

    /**
     * Function to create a question.
     *
     * @param smartquest $smartquest
     * @param array|stdClass $record
     * @param array|stdClass $data - accompanying data for question - e.g. choices
     * @return \mod_smartquest\question\base the question object
     */
    public function create_question(smartquest $smartquest, $record = null, $data = null) {
        global $DB;
        ...
        $record['position'] = count($smartquest->questions);
        ...
        if ($typeid !== QUESPAGEBREAK && $typeid !== QUESSECTIONTEXT) {
            $qtype = $DB->get_record('smartquest_question_type', ['id' => $typeid]);
            if (!$qtype) {
        ...
        // Add the question.
        $record->id = $DB->insert_record('smartquest_question', $record);

        $question = \mod_smartquest\question\base::question_builder($record->type_id, $record->id, $record);
        ...
        // Update smartquest.
        $smartquest->add_questions();

        return $question;
    }

    /**
     * Create a smartquest with questions and response data for use in other tests.
     */
    public function create_test_smartquest($course, $qtype = null, $questiondata = array(), $choicedata = null) {
        $smartquest = $this->create_instance(array('course' => $course->id));
        ...
        if ($qtype !== null) {
            ...
            $this->create_question($smartquest, $questiondata, $choicedata);
        }
     ...
     */
    public function create_question_response($smartquest, $question, $respval, $userid = 1, $section = 1) {
        global $DB;
        $currentrid = 0;
        $_POST['q'.$question->id] = $respval;
        $responseid = $smartquest->response_insert($question->survey_id, $section, $currentrid, $userid);
        $this->response_commit($smartquest, $responseid);
        smartquest_record_submission($smartquest, $userid, $responseid);
        return $DB->get_record('smartquest_response', array('id' => $responseid));
    }

    /**
     * Need to create a method to access a private smartquest method.
     * TO DO - may not need this with above "TO DO".
     */
    private function response_commit($smartquest, $responseid) {
        $method = new ReflectionMethod('smartquest', 'response_commit');
        $method->setAccessible(true);
        return $method->invoke($smartquest, $responseid);
    }
     ...
     *
     * @param \mod_smartquest\question\base $question
     * @param stdClass $data
     */
    ...
    protected function add_response_choice($questionresponse, $responseid) {
        ...
        $question = $DB->get_record('smartquest_question', ['id' => $questionresponse->questionid]);
        ...
        if ($qtype === QUESCHOOSE || $qtype === QUESRADIO || $qtype === QUESDROP || $qtype === QUESCHECK || $qtype === QUESRATE) {
            if (is_int($questionresponse->response)) {
                ...
                $rs = $DB->get_records_sql("SELECT * FROM {smartquest_quest_choice} $select", $params, 0, 1);
                $choice = reset($rs);
                if (!$choice) {
            ...
            if ($qtype == QUESRATE) {
                $DB->insert_record('smartquest_response_rank', [
                ...
                if ($qtype === QUESCHOOSE || $qtype === QUESRADIO || $qtype === QUESDROP) {
                    $instable = 'smartquest_resp_single';
                } else if ($qtype === QUESCHECK) {
                    $instable = 'smartquest_resp_multiple';
                }
            ...
            $DB->insert_record('smartquest_response_text', [
                    'response_id' => $responseid,
                    'question_id' => $questionresponse->questionid,
    ...
    public function create_response($record = null, $questionresponses) {
        ...
        // Add the response.
        $record['id'] = $DB->insert_record('smartquest_response', $record);
        ...
        $attempt = ['qid' => $record['survey_id'], 'userid' => $record['userid'], 'rid' => $record['id'],
            'timemodified' => time()];
        $DB->insert_record('smartquest_attempts', $attempt);
    ...
    /**
     * @param smartquest $smartquest
     * @param \mod_smartquest\question\base[] $questions
     * @param $userid
     * @return stdClass
     * @throws coding_exception
     */
    public function generate_response($smartquest, $questions, $userid) {
        $responses = [];
        ...
        return $this->create_response(['survey_id' => $smartquest->sid, 'userid' => $userid], $responses);
    }

    public function create_and_fully_populate($coursecount = 4, $studentcount = 20, $smartquestcount = 2,
            $questionspertype = 5) {
        global $DB;
        ...
        // Create smartquests in each course.
        $qname = 1000;
        for ($q = 0; $q < $smartquestcount; $q++) {
            foreach ($courses as $course) {
                $smartquest = $qdg->create_instance(['course' => $course->id]);
                $smartquests[] = $smartquest;
                $questions = [];
                ...
                // Create responses.
                foreach ($students as $student) {
                    $qdg->generate_response($smartquest, $questions, $student->id);
                }
            }
        }
